...
(defn main [] (let [argc (command-line-args)] (if (= argc 0) (show-help) (let [cmd-name (command-line-arg 0) file-path (if (> argc 1) (command-line-arg 1) "") flag (if (> argc 2) (command-line-arg 2) "") output-path (if (> argc 3) (command-line-arg 3) "")] (if (and (string-eq cmd-name "lsp") (string-eq file-path "--stdio")) (run-lsp-stdio-server) (if (and (> argc 3) (output-option-flag flag)) (if (string-eq cmd-name "compile") (run-compile-output file-path output-path) (if (string-eq cmd-name "build") (run-build-output file-path output-path) (run-command cmd-name file-path 0))) (run-command cmd-name file-path 0)))))))
