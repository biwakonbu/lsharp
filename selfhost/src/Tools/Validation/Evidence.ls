...
(defn validation-json-field [name value-json]
  (do
    (root_push name)
    (root_push value-json)
    (let [colon-value (string-concat "\":" value-json)]
      (do
        (root_push colon-value)
        (let [name-colon (string-concat name colon-value)]
          (do
            (root_push name-colon)
            (let [result (string-concat "\"" name-colon)]
              (do
                (root_push result)
                (root_pop)
                (root_pop)
...
(defn validation-source-node-json-state-loop [state]
  (do
    (root_push state)
    (let [node (vector-get state 0)
      stage (vector-get state 1)
      fields (vector-get state 2)]
      (if (= stage 0)
        (let [next-fields (validation-json-append fields
                      ...
                      (do
                        ...
                        (let [result (validation-source-node-json-state-loop next-state)]
                          (do
                            ...
                            (root_pop)
                            result))))))
                (let [result (validation-json-object-wrap fields)]
                  (do
                    (root_push result)
                    (root_pop)
                    (root_pop)
...
(defn validation-source-subject-json [subject]
  (do
    ...
    (let [fields0 (validation-json-string-field "kind" (validation-source-subject-kind-text subject))]
      (do
        (root_push fields0)
        (let [namespace-field (validation-json-string-field "namespace"
            (let [wire-id subject
              colon (source-find-char wire-id 58 0 (string-length wire-id))
              slash (source-find-char wire-id 47 (+ colon 1) (string-length wire-id))]
              (substring wire-id (+ colon 1) slash)))]
          (do
            (root_push namespace-field)
            (let [key-field (validation-json-string-field "key"
                (let [wire-id subject
                  colon (source-find-char wire-id 58 0 (string-length wire-id))
                  slash (source-find-char wire-id 47 (+ colon 1) (string-length wire-id))]
                  (substring wire-id (+ slash 1) (string-length wire-id))))]
              (do
                (root_push key-field)
                (let [comma-key-fields (string-concat "," key-field)]
                  (do
                    (root_push comma-key-fields)
                    (let [id-object-fields (string-concat namespace-field comma-key-fields)]
                      (do
