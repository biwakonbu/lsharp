...
(defn make-parse-loop-state [done value]
  (vector-push (vector-push (vector-new 2) done) value))

(defn make-parser-scan-state [done next-pos value]
  (vector-push
    (vector-push
...
(defn name-hash-step-v3 [src pos end acc]
  (if (>= pos end)
    (make-parser-scan-state 1 pos acc)
    (let [next-pos (+ pos 1)
      next-acc (+ (string-char-at src pos) (* acc 31))]
      (make-parser-scan-state (if (>= next-pos end) 1 0) next-pos next-acc))))
  ...
  (let [step (name-hash-step-v3 src pos end acc)
    ...
    (if (= done 1)
      ...
      (if (<= remaining 1)
        ...
        (do
          (root_push step)
          (let [parsed (name-hash-step-64-loop-bounded
                         src next-pos end next-acc (- remaining 1))]
            (do
              (root_pop)
              parsed)))))))
...
(defn name-hash-rooted-v3 [src pos end acc]
  (let [step (name-hash-step-64-loop-bounded src pos end acc 64)
    ...
    (if (= done 1)
      ...
      (do
        (root_push step)
        (let [parsed (name-hash-rooted-v3 src next-pos end next-acc)]
          (do
            (root_pop)
            parsed))))))
...
(defn name-hash-loop [src pos end acc]
  (do
    (root_push src)
    (let [result (name-hash-rooted-v3 src pos end acc)]
      (do
        (root_pop)
        result))))
  ...
  (let [step (symbol-dot-position-step-v3 src pos end)
    ...
    (if (= done 1)
      ...
      (if (<= remaining 1)
        ...
        (do
          (root_push step)
          (let [parsed (symbol-dot-position-step-64-loop-bounded
                         src next-pos end (- remaining 1))]
            (do
              (root_pop)
              parsed)))))))
...
(defn symbol-dot-position-rooted-v3 [src pos end]
  (let [step (symbol-dot-position-step-64-loop-bounded src pos end 64)
    ...
    (if (= done 1)
      ...
      (do
        (root_push step)
        (let [parsed (symbol-dot-position-rooted-v3 src next-pos end)]
          (do
            (root_pop)
            parsed))))))
...
(defn symbol-dot-position-loop [src pos end]
  (do
    (root_push src)
    (let [result (symbol-dot-position-rooted-v3 src pos end)]
      (do
        (root_pop)
        result))))
...
(defn parse-int-digits-from-str-step-v3 [src pos end acc]
  (if (>= pos end)
    (make-parser-scan-state 1 pos acc)
    (let [digit (- (string-char-at src pos) 48)
      next-pos (+ pos 1)
      next-acc (+ (* acc 10) digit)]
      (make-parser-scan-state (if (>= next-pos end) 1 0) next-pos next-acc))))

(defn parse-int-digits-from-str-step-64-loop-bounded [src pos end acc remaining]
  (let [step (parse-int-digits-from-str-step-v3 src pos end acc)
    done (vector-get step 0)
    next-pos (vector-get step 1)
    next-acc (vector-get step 2)]
    (if (= done 1)
      ...
      (if (<= remaining 1)
        ...
        (do
          (root_push step)
          (let [parsed (parse-int-digits-from-str-step-64-loop-bounded
                         src next-pos end next-acc (- remaining 1))]
            (do
              (root_pop)
              parsed)))))))

(defn parse-int-digits-from-str-rooted-v3 [src pos end acc]
  (let [step (parse-int-digits-from-str-step-64-loop-bounded src pos end acc 64)
    done (vector-get step 0)
    next-pos (vector-get step 1)
    next-acc (vector-get step 2)]
    (if (= done 1)
      ...
      (do
        (root_push step)
        (let [parsed (parse-int-digits-from-str-rooted-v3
                       src next-pos end next-acc)]
          (do
            (root_pop)
            parsed))))))
...
(defn parse-int-digits-from-str [src pos end acc]
  (do
    (root_push src)
    (let [result (parse-int-digits-from-str-rooted-v3 src pos end acc)]
      (do
        (root_pop)
        result))))
      ...
      (if (= char 92)
        (if (< (+ pos 1) end)
          (let [escaped (string-char-at src (+ pos 1))
            next-pos (+ pos 2)
            next-acc (+ (string-literal-map-hash-escaped-char escaped) (* acc 31))]
            (make-parser-scan-state
              (if (>= next-pos end) 1 0)
              next-pos
              next-acc))
          (let [next-pos (+ pos 1)
            next-acc (+ char (* acc 31))]
            (make-parser-scan-state (if (>= next-pos end) 1 0) next-pos next-acc)))
        (let [next-pos (+ pos 1)
          next-acc (+ char (* acc 31))]
          (make-parser-scan-state (if (>= next-pos end) 1 0) next-pos next-acc))))))

(defn string-literal-map-hash-step-64-loop-bounded [src pos end acc remaining]
  (let [step (string-literal-map-hash-step-v3 src pos end acc)
    ...
    (if (= done 1)
      ...
      (if (<= remaining 1)
        ...
        (do
          (root_push step)
          (let [parsed (string-literal-map-hash-step-64-loop-bounded
                         src next-pos end next-acc (- remaining 1))]
            (do
              (root_pop)
              parsed)))))))
...
(defn string-literal-map-hash-rooted-v3 [src pos end acc]
  (let [step (string-literal-map-hash-step-64-loop-bounded src pos end acc 64)
    done (vector-get step 0)
    next-pos (vector-get step 1)
    next-acc (vector-get step 2)]
    (if (= done 1)
      ...
      (do
        (root_push step)
        (let [parsed (string-literal-map-hash-rooted-v3 src next-pos end next-acc)]
          (do
            (root_pop)
            parsed))))))
...
(defn string-literal-map-hash-loop [src pos end acc]
  (do
    (root_push src)
    (let [result (string-literal-map-hash-rooted-v3 src pos end acc)]
      (do
        (root_pop)
        result))))
  ...
  (if (>= i len)
    ...
    (do
      ...
      (let [next-result
        ...
        (let [state (make-vector-set-at-state 0 (+ i 1) next-result)]
          (do
            (root_pop)
            (root_pop)
            (root_pop)
...
(defn vector-set-at-step-64-loop-bounded [vec result idx new-val i len remaining]
  (do
    (root_push vec)
    (root_push result)
    (root_push new-val)
...
(defn vector-set-at-step-64 [vec result idx new-val i len]
  (vector-set-at-step-64-loop-bounded vec result idx new-val i len 64))

(defn vector-set-at-loop [vec result idx new-val i len]
  (let [step (vector-set-at-step-64 vec result idx new-val i len)]
      ...
      (do
        ...
        (let [next-index (vector-get step 1)
          ...
          (do
            (root_push next-result)
            (let [parsed
              (vector-set-at-loop vec next-result idx new-val next-index len)]
              (do
                (root_pop)
                (root_pop)
                parsed))))))))
...
(defn parse-skip-brace-step-64-loop-bounded [spans pos-ref depth remaining]
...
(defn parse-skip-brace-step-64 [spans pos-ref depth]
  (parse-skip-brace-step-64-loop-bounded spans pos-ref depth 64))

(defn parse-skip-brace-rooted-v3 [spans pos-ref depth]
  (let [step (parse-skip-brace-step-64 spans pos-ref depth)]
    (if (= (vector-get step 0) 1)
      0
      (do
        (root_push step)
        (let [next-depth (vector-get step 1)
          parsed (parse-skip-brace-rooted-v3 spans pos-ref next-depth)]
          (do
            (root_pop)
            parsed))))))
...
(defn parse-skip-brace-v3 [spans pos-ref depth]
  (do
    (root_push spans)
    (root_push pos-ref)
    (let [parsed (parse-skip-brace-rooted-v3 spans pos-ref depth)]
      (do
        (root_pop)
        (root_pop)
        parsed))))
...
(defn defn-signature-param-present-step-v3 [signature idx count]
  (if (>= idx count)
    (make-parser-scan-state 1 idx 0)
    (if (= (vector-get signature (+ idx 2)) 0)
      (make-parser-scan-state 0 (+ idx 1) 0)
      (make-parser-scan-state 1 idx 1))))
...
(defn defn-signature-param-present-step-64 [signature idx count]
  (defn-signature-param-present-step-64-loop-bounded signature idx count 64))

(defn defn-signature-param-present-v3 [signature idx count]
  (let [step (defn-signature-param-present-step-64 signature idx count)]
    (if (= (vector-get step 0) 1)
      (vector-get step 2)
      (do
        (root_push step)
        (let [next-index (vector-get step 1)]
          (let [parsed
            (defn-signature-param-present-v3 signature next-index count)]
            (do
              (root_pop)
              parsed)))))))
