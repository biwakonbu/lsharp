        ...
        (if (= target-param-count 3)
          ...
          (if (= target-param-count 2)
            ...
            (if (= target-param-count 1)
              ...
              5))))))
    (if (= opcode 3)
      (if (>= current-depth 4) 43 (if (>= current-depth 3) 29 (if (>= current-depth 2) 15 8)))
      (if (= opcode 10)
        (if (>= current-depth 4) 45 (if (>= current-depth 3) 31 (if (>= current-depth 2) 17 10)))
        (if (= opcode 44)
          (if (>= current-depth 5) 38 (if (>= current-depth 4) 24 (if (>= current-depth 3) 10 3)))
...
(defn native-function-size-x86 [func-meta function-metas]
  (let [param-count (native-function-param-count func-meta)
    ...
    param-spill-bytes (if (= param-count 5)
                        35
                        (if (= param-count 4)
                          28
                          (if (= param-count 3)
          ...
          (if (= target-param-count 3)
            ...
            (if (= target-param-count 2)
              ...
              (if (= target-param-count 1)
                ...
                4))))))
    (if (= opcode 3)
      (if (>= current-depth 4) 28 (if (>= current-depth 3) 20 (if (>= current-depth 2) 12 8)))
      (if (= opcode 10)
        (if (>= current-depth 4) 28 (if (>= current-depth 3) 20 (if (>= current-depth 2) 12 8)))
        (if (= opcode 44)
          (if (>= current-depth 5) 24 (if (>= current-depth 4) 16 (if (>= current-depth 3) 8 4)))
