...
;; 現状の partial slice では 17-value window ぶんまで spill slot を確保する
...
(defn native-function-size-x86 [func-meta function-metas]
  (let [param-count (native-function-param-count func-meta)
    ...
    param-spill-bytes (if (= param-count 17)
                        163
                        (if (= param-count 16)
                        152
                        (if (= param-count 15)
