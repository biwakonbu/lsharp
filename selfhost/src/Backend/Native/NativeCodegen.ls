...
;; 現状の partial slice では 18-value window ぶんまで spill slot を確保する
...
(defn native-function-size-x86 [func-meta function-metas]
  (let [param-count (native-function-param-count func-meta)
    ...
    param-spill-bytes (if (= param-count 19)
                        185
                        (if (= param-count 18)
                        174
                        (if (= param-count 17)
