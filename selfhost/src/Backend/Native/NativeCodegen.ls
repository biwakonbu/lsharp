...
;; 現状の partial slice では 16-value window ぶんまで spill slot を確保する
...
(defn native-function-size-x86 [func-meta function-metas]
  (let [param-count (native-function-param-count func-meta)
    ...
    param-spill-bytes (if (= param-count 16)
                        152
                        (if (= param-count 15)
                         141
                         (if (= param-count 14)
