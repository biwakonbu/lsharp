...
(defn emit-import-section-wasi-standalone [func-count]
  (let [body0 (emit-byte (vector-new 224) 7)
    ...
    body31 (emit-standalone-byte-seq-8 body30 22 119 97 115 105 95 115 110)
    body32 (emit-standalone-byte-seq-8 body31 97 112 115 104 111 116 95 112)
    body33 (emit-standalone-byte-seq-8 body32 114 101 118 105 101 119 49 9)
    body34 (emit-standalone-byte-seq-8 body33 112 114 111 99 95 101 120 105)
    body35a (emit-standalone-byte-seq-2 body34 116 0)
    body35 (emit-leb128 body35a (+ func-count 11))
    body-size (vector-length body35)
    result0 (emit-byte (vector-new 64) 2)
    result1 (emit-leb128 result0 body-size)]
    (append-byte-vector-chunked result1 body35 0 body-size)))
...
(defn emit-standalone-print-body []
  (let [b0 (emit-byte (vector-new 256) 2)
    b1 (emit-standalone-byte-seq-8 b0 1 126 6 127 32 0 33 1)
    ;; local7 は符号を保持し、local1 は絶対値へ変換してから数字化する。
    b2 (emit-standalone-byte-seq-8 b1 65 0 33 7 32 1 66 0)
    b3 (emit-standalone-byte-seq-8 b2 83 4 64 65 1 33 7 66)
    b4 (emit-standalone-byte-seq-8 b3 0 32 1 125 33 1 11 65)
    b5 (emit-standalone-byte-seq-8 b4 172 2 33 3 65 0 33 2)
    b6 (emit-standalone-byte-seq-8 b5 2 64 3 64 32 1 80 13)
    b7 (emit-standalone-byte-seq-8 b6 1 32 3 32 2 106 32 1)
    b8 (emit-standalone-byte-seq-8 b7 66 10 130 167 65 48 106 58)
    b9 (emit-standalone-byte-seq-8 b8 0 0 32 1 66 10 128 33)
    b10 (emit-standalone-byte-seq-8 b9 1 32 2 65 1 106 33 2)
    b11 (emit-standalone-byte-seq-8 b10 12 0 11 11 32 7 4 64)
    ...
    b19 (emit-standalone-byte-seq-4 b18 24 16 0 26)
...
(defn emit-standalone-read-stdin-body []
  ;; stdin は容量倍増する単一 String bufferへ読み、chunkごとの二次的な再確保を避ける。
  ...
  (let [
    ...
    b0a (emit-leb128-s b0 4104)
    b1 (emit-standalone-byte-seq-8 b0a 16 1 33 4 32 4 167 65)
    b2 (emit-standalone-byte-seq-8 b1 1 54 2 0 32 4 167 65)
    b2a (emit-standalone-byte-seq-4 b2 0 54 2 4)
    b3a (emit-standalone-byte-seq-4 b2a 65 0 33 3)
    b3b (emit-standalone-byte-seq-1 b3a 65)
    b3c (emit-leb128-s b3b 4096)
    b4 (emit-standalone-byte-seq-8 b3c 33 7 65 0 33 8 2 64)
    b5 (emit-standalone-byte-seq-8 b4 3 64 32 8 32 7 79 4)
    b5a (emit-standalone-byte-seq-1 b5 64)
    b6 (emit-standalone-byte-seq-8 b5a 32 4 33 5 32 7 32 7)
    b6a (emit-standalone-byte-seq-1 b6 106)
    b6b (emit-standalone-byte-seq-8 b6a 33 7 32 7 65 8 106 173)
    b6c (emit-standalone-byte-seq-4 b6b 16 1 33 4)
    b7 (emit-standalone-byte-seq-8 b6c 32 4 167 65 1 54 2 0)
    b8 (emit-standalone-byte-seq-8 b7 32 4 167 32 8 54 2 4)
    b9 (emit-standalone-byte-seq-8 b8 32 4 167 65 8 106 32 5)
    b10 (emit-standalone-byte-seq-8 b9 167 65 8 106 32 8 252 10)
    b10a (emit-standalone-byte-seq-2 b10 0 0)
    b11 (emit-standalone-byte-seq-1 b10a 11)
    b12 (emit-standalone-byte-seq-1 b11 65)
    b12a (emit-leb128-s b12 2176)
    b13 (emit-standalone-byte-seq-8 b12a 32 4 167 65 8 106 32 8)
    b13a (emit-standalone-byte-seq-4 b13 106 54 2 0)
    b14 (emit-standalone-byte-seq-1 b13a 65)
    b14a (emit-leb128-s b14 2176)
    b14b (emit-standalone-byte-seq-1 b14a 65)
    b14c (emit-leb128-s b14b 4096)
    b15 (emit-standalone-byte-seq-4 b14c 54 2 4 65)
    b15a (emit-leb128-s b15 2184)
    b16 (emit-standalone-byte-seq-8 b15a 65 0 54 2 0 32 3 65)
    b16a (emit-leb128-s b16 2176)
    b17a (emit-standalone-byte-seq-2 b16a 65 1)
    b17b (emit-standalone-byte-seq-1 b17a 65)
    b17c (emit-leb128-s b17b 2184)
    b18a (emit-standalone-byte-seq-2 b17c 16 15)
    b18 (emit-standalone-byte-seq-1 b18a 26)
    b19 (emit-standalone-byte-seq-1 b18 65)
    b19a (emit-leb128-s b19 2184)
    b20 (emit-standalone-byte-seq-8 b19a 40 2 0 33 6 32 6 69)
    b21 (emit-standalone-byte-seq-2 b20 13 1)
    b22 (emit-standalone-byte-seq-6 b21 32 8 32 6 106 33)
    b22a (emit-standalone-byte-seq-1 b22 8)
    b23 (emit-standalone-byte-seq-8 b22a 32 4 167 32 8 54 2 4)
    b24 (emit-standalone-byte-seq-2 b23 12 0)
    b25 (emit-standalone-byte-seq-1 b24 11)
    b26 (emit-standalone-byte-seq-1 b25 11)
    b27 (emit-standalone-byte-seq-2 b26 32 4)
    b28 (emit-standalone-byte-seq-1 b27 11)]
    b28))
...
(defn emit-standalone-read-file-body []
  ...
  (let [b0a (emit-standalone-byte-seq-2 (vector-new 1024) 1 8)
    ...
    b3 (emit-standalone-byte-seq-8 b2 65 128 32 173 16 1 167 33)
    ...
    b16 (emit-standalone-byte-seq-8 b15a 65 128 17 65 128 32 32 6)
    ...
    b19 (emit-standalone-byte-seq-8 b18 136 17 16 15 33 3 32 3)
    b19a (emit-standalone-byte-seq-8 b19 4 64 65 0 33 6 12 2)
    b19b (emit-standalone-byte-seq-1 b19a 11)
    b20 (emit-standalone-byte-seq-8 b19b 65 136 17 40 2 0 33 7)
    b21 (emit-standalone-byte-seq-8 b20 32 7 69 13 1 32 7 32)
    b22 (emit-standalone-byte-seq-4 b21 6 106 33 6)
...
(defn emit-standalone-read-file-body-chunked []
  ...
  (let [
    ...
    b1 (emit-standalone-byte-seq-8 b0 0 167 65 8 106 33 1 32)
    b2 (emit-standalone-byte-seq-8 b1 0 167 40 2 4 33 2 65)
    b3 (emit-standalone-byte-seq-8 b2 3 65 0 32 1 32 2 65)
    b4 (emit-standalone-byte-seq-8 b3 0 66 2 66 0 65 0 65)
    b5 (emit-standalone-byte-seq-8 b4 192 17 16 13 33 8 66 8)
    b6 (emit-standalone-byte-seq-8 b5 16 1 33 4 32 4 167 65)
    ...
    b19 (emit-standalone-byte-seq-8 b18 7 69 4 64 65 136 17 40)
    b20 (emit-standalone-byte-seq-8 b19 2 0 33 6 5 65 0 33)
    b21 (emit-standalone-byte-seq-8 b20 6 11 32 6 69 13 1 32)
    b22 (emit-standalone-byte-seq-8 b21 5 167 32 6 54 2 4 32)
    b23 (emit-standalone-byte-seq-8 b22 4 32 5 16 5 33 4 12)
    b23a (emit-standalone-byte-seq-2 b23 0 11)
    b24 (emit-standalone-byte-seq-8 b23a 11 32 3 16 14 33 8 32)
    b25 (emit-standalone-byte-seq-8 b24 7 69 4 64 5 66 8 16)
    b26 (emit-standalone-byte-seq-8 b25 1 33 4 32 4 167 65 1)
    b27 (emit-standalone-byte-seq-8 b26 54 2 0 32 4 167 65 0)
    b28 (emit-standalone-byte-seq-8 b27 54 2 4 11 32 8 69 4)
    b29 (emit-standalone-byte-seq-8 b28 64 5 66 8 16 1 33 4)
    b30 (emit-standalone-byte-seq-8 b29 32 4 167 65 1 54 2 0)
    b31 (emit-standalone-byte-seq-8 b30 32 4 167 65 0 54 2 4)
    b32 (emit-standalone-byte-seq-4 b31 11 11 32 4)
    b33 (emit-standalone-byte-seq-1 b32 11)
    ]
    b33))
    ...
    (if (= idx 1)
      ...
      (if (= idx 11)
        ...
        (if (= idx 12)
          ...
          (if (= idx 13)
            (emit-standalone-read-file-body-chunked)
            (if (= idx 14)
...
(defn emit-type-section-wasi-standalone [functions]
  (let [func-count (vector-length functions)
    ...
    body17 (emit-standalone-byte-seq-4 body16 96 1 127 0)
    body-size (vector-length body17)
