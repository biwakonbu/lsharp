...
;; 7-import (alloc/print/read-file/command-line-arg/string-concat/substring/file-exists?) + memory + data モデル用セクション生成関数群
;; タイプセクション: type0=(i64->i64), type1=(i64->void), type2=(i64×2->i64), type3=(i64×3->i64),
;;   type4..(4+N-1)=ユーザ関数型, type4+N=_start型
(defn emit-type-section-wasi-quad-functions [functions] (let [func-count (vector-length functions) total-count (+ func-count 5) body0 (emit-leb128 (vector-new 64) total-count) body1 (emit-byte body0 96) body2 (emit-leb128 body1 1) body3 (emit-byte body2 126) body4 (emit-byte body3 1) body5 (emit-byte body4 126) body6 (emit-byte body5 96) body7 (emit-leb128 body6 1) body8 (emit-byte body7 126) body9 (emit-leb128 body8 0) body10 (emit-byte body9 96) body11 (emit-leb128 body10 2) body12 (emit-byte body11 126) body13 (emit-byte body12 126) body14 (emit-byte body13 1) body15 (emit-byte body14 126) body16 (emit-byte body15 96) body17 (emit-leb128 body16 3) body18 (emit-byte body17 126) body19 (emit-byte body18 126) body20 (emit-byte body19 126) body21 (emit-byte body20 1) body22 (emit-byte body21 126) body23 (append-function-types body22 functions 0 func-count) body24 (emit-byte body23 96) body25 (emit-leb128 body24 0) body26 (emit-leb128 body25 0) body-size (vector-length body26) result0 (emit-byte (vector-new 64) 1) result1 (emit-leb128 result0 body-size)] (append-byte-vector result1 body26 0 body-size)))
;; 関数セクション: ユーザ関数 type4..(4+N-1) + _start type(4+N)
(defn emit-function-section-wasi-quad-functions [functions] (let [func-count (vector-length functions) body0 (emit-leb128 (vector-new 32) (+ func-count 1)) body1 (append-type-index-sequence body0 4 (+ 4 func-count)) body2 (emit-leb128 body1 (+ 4 func-count)) body-size (vector-length body2) result0 (emit-byte (vector-new 32) 3) result1 (emit-leb128 result0 body-size)] (append-byte-vector result1 body2 0 body-size)))
;; コードセクション: ユーザ関数ボディ + _start(import7個後の最終ユーザ関数=6+func_count を call)
(defn emit-code-section-wasi-quad-functions [functions] (let [func-count (vector-length functions) main-func-idx (+ 6 func-count) body0 (emit-leb128 (vector-new 64) (+ func-count 1)) body1 (append-code-bodies-functions body0 functions 0 func-count) wrapper0 (emit-byte (vector-new 8) 0) wrapper1 (emit-byte wrapper0 16) wrapper2 (emit-leb128 wrapper1 main-func-idx) wrapper3 (emit-byte wrapper2 26) wrapper4 (emit-byte wrapper3 11) wrapper-size (vector-length wrapper4) body2 (emit-leb128 body1 wrapper-size) body3 (append-byte-vector body2 wrapper4 0 wrapper-size) body-size (vector-length body3) result0 (emit-byte (vector-new 64) 10) result1 (emit-leb128 result0 body-size)] (append-byte-vector result1 body3 0 body-size)))
...
;; 7-import セクション: alloc(type0), print(type1), read-file(type0), command-line-arg(type0), string-concat(type2), substring(type3), file-exists?(type0)
...
(defn emit-import-section-alloc-print-read-arg-concat-sub [] (let [body0 (emit-leb128 (vector-new 128) 7) body1 (append-import-alloc-entry body0) body2 (append-import-print-entry body1) body3 (append-import-read-file-entry body2) body4 (append-import-command-line-arg-entry body3) body5 (append-import-string-concat-entry body4) body6 (append-import-substring-entry body5) body7 (append-import-file-exists-entry body6) body-size (vector-length body7) result0 (emit-byte (vector-new 128) 2) result1 (emit-leb128 result0 body-size)] (append-byte-vector result1 body7 0 body-size)))
...
(defn emit-root-push-instr [bytes] (let [b1 (emit-byte bytes 26) b2 (emit-byte b1 66)] (emit-leb128-s b2 0)))
(defn emit-root-pop-instr [bytes] (emit-leb128-s (emit-byte bytes 66) 0))
(defn emit-root-set-instr [bytes] (let [b1 (emit-byte bytes 26) b2 (emit-byte b1 26) b3 (emit-byte b2 66)] (emit-leb128-s b3 0)))
