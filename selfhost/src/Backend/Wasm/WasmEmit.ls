...
(defn standalone-write-file-bytes-iovec-address [] 2048)
(defn standalone-write-file-bytes-nwritten-address [] 2056)
...
(defn emit-standalone-write-file-body []
  ;; create|truncate + single fd_write の bounded slice。open failure は -1 を返す。
  (let [b0a (emit-standalone-byte-seq-2 (vector-new 512) 1 7)
    b0 (emit-byte b0a 127)
    ...
    b9 (emit-standalone-byte-seq-8 b8 65 0 65 152 2 16 13 33)
    ...
    b13 (emit-standalone-byte-seq-8 b12 32 6 69 4 64 65 152 2)
    b14 (emit-standalone-byte-seq-8 b13 40 2 0 33 7 65 224 2)
    b15 (emit-standalone-byte-seq-8 b14 32 4 54 2 0 65 224 2)
    b16 (emit-standalone-byte-seq-8 b15 32 5 54 2 4 65 232 2)
    b17 (emit-standalone-byte-seq-8 b16 65 0 54 2 0 32 7 65)
    b18 (emit-standalone-byte-seq-8 b17 224 2 65 1 65 232 2 16)
    b19 (emit-standalone-byte-seq-8 b18 0 26 65 232 2 40 2 0)
    b20 (emit-standalone-byte-seq-8 b19 33 8 32 7 16 14 26 11)
    b21 (emit-standalone-byte-seq-4 b20 32 8 172 11)]
    b21))
(defn emit-standalone-write-file-bytes-body []
  ;; Vector の下位8 bitを packed buffer へ詰めて一回 fd_write する bounded slice。
  (let [b0a (emit-standalone-byte-seq-2 (vector-new 512) 1 8)
    b0 (emit-byte b0a 127)
    b1 (emit-standalone-byte-seq-8 b0 32 0 167 65 8 106 33 2)
    b2 (emit-standalone-byte-seq-8 b1 32 0 167 40 2 4 33 3)
    ...
    b18 (emit-standalone-byte-seq-8 b17e 65 0 65 152 2 16 13 26)
    b19 (emit-standalone-byte-seq-8 b18 65 152 2 40 2 0 33 8)
    b20a (emit-byte b19 65)
    b20b (emit-leb128 b20a (standalone-write-file-bytes-iovec-address))
    b20c (emit-standalone-byte-seq-4 b20b 32 6 54 2)
    b20d (emit-byte b20c 0)
    b21a (emit-byte b20d 65)
    b21b (emit-leb128 b21a (standalone-write-file-bytes-iovec-address))
    b21c (emit-standalone-byte-seq-4 b21b 32 5 54 2)
    b21d (emit-byte b21c 4)
    b22 (emit-standalone-byte-seq-2 b21d 32 8)
    b22a (emit-byte b22 65)
    b22b (emit-leb128 b22a (standalone-write-file-bytes-iovec-address))
    b22c (emit-byte b22b 65)
    b22d (emit-leb128 b22c 1)
    b22e (emit-byte b22d 65)
    b22f (emit-leb128 b22e (standalone-write-file-bytes-nwritten-address))
    b22g (emit-standalone-byte-seq-2 b22f 16 0)
    b22h (emit-byte b22g 26)
    b23a (emit-byte b22h 65)
    b23b (emit-leb128 b23a (standalone-write-file-bytes-nwritten-address))
    b23c (emit-standalone-byte-seq-4 b23b 40 2 0 33)
    b23d (emit-byte b23c 9)
    b24 (emit-standalone-byte-seq-4 b23d 32 8 16 14)
    b24a (emit-byte b24 26)
    b25 (emit-standalone-byte-seq-4 b24a 32 9 173 11)]
    b25))
