...
;; 6-import (alloc/print/read-file/command-line-arg/string-concat/substring) + memory + data モデル用セクション生成関数群
...
;; コードセクション: ユーザ関数ボディ + _start(import6個後の最終ユーザ関数=5+func_count を call)
(defn emit-code-section-wasi-quad-functions [functions] (let [func-count (vector-length functions) main-func-idx (+ 5 func-count) body0 (emit-leb128 (vector-new 64) (+ func-count 1)) body1 (append-code-bodies-functions body0 functions 0 func-count) wrapper0 (emit-byte (vector-new 8) 0) wrapper1 (emit-byte wrapper0 16) wrapper2 (emit-leb128 wrapper1 main-func-idx) wrapper3 (emit-byte wrapper2 26) wrapper4 (emit-byte wrapper3 11) wrapper-size (vector-length wrapper4) body2 (emit-leb128 body1 wrapper-size) body3 (append-byte-vector body2 wrapper4 0 wrapper-size) body-size (vector-length body3) result0 (emit-byte (vector-new 64) 10) result1 (emit-leb128 result0 body-size)] (append-byte-vector result1 body3 0 body-size)))
...
(defn emit-wasm-with-target [ir-instrs target] (emit-wasm ir-instrs))
...
;; 6-import セクション: alloc(type0), print(type1), read-file(type0), command-line-arg(type0), string-concat(type2), substring(type3)
...
(defn emit-import-section-alloc-print-read-arg-concat-sub [] (let [body0 (emit-leb128 (vector-new 128) 6) body1 (append-import-alloc-entry body0) body2 (append-import-print-entry body1) body3 (append-import-read-file-entry body2) body4 (append-import-command-line-arg-entry body3) body5 (append-import-string-concat-entry body4) body6 (append-import-substring-entry body5) body-size (vector-length body6) result0 (emit-byte (vector-new 128) 2) result1 (emit-leb128 result0 body-size)] (append-byte-vector result1 body6 0 body-size)))
