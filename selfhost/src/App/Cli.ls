...
(defn check-exit-code [diagnostics-count] (if (> diagnostics-count 0) (exit-code-compile-error) (exit-success)))
...
(defn lsp-type-diagnostic-to-lsp [code src start end]
  (let [has-span (or (= code (error-code-undefined)) (or (= code (error-code-if-branch)) (or (= code (error-code-arg-mismatch)) (= code (error-code-infinite)))))
...
(defn validate-help-text [] "validate --source <file> --format <text|json> [--emit-manifest <output.json>] - Validate source intent graph")
...
(defn run-main-command [argc cmd-name file-path]
  (if (string-eq cmd-name "test")
    (run-test-command argc cmd-name file-path)
    (run-main-command-core argc cmd-name file-path)))
