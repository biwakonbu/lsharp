      ...
      (let [next-idx (vector-get state 1)
		...
		(defn compile-defn-with-source-production-inner-diagnostic [node source ftable data-ref]
		  (do
		    ...
		    (root_push data-ref)
		    (print 9000000319)
		    (print (vector-get (vector-get node (+ 3 (vector-get node 2))) 0))
		    (print (vector-length (vector-get node (+ 3 (vector-get node 2)))))
	    ...
	    (let [decls-slot (root_push decls)
	      ...
	      (if (= (vector-get decl 0) 20)
	        (do
	          (root_push decl)
	          (print 9000000303)
	          (print (vector-get (vector-get decl (+ 3 (vector-get decl 2))) 0))
	          (print (vector-length (vector-get decl (+ 3 (vector-get decl 2)))))
	          (print 9000000300)
	          (print idx)
	          (print n)
	          (print (vector-length functions))
