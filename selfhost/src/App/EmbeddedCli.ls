...
(defn run-compile-output [file-path output-path opts] (if (file-exists? file-path) (if (= opts (compile-target-preview1)) (let [wasm-bytes (compile-file-wasm-bytes file-path) summary (wasm-size-text (vector-length wasm-bytes))] (do (write-file-bytes output-path wasm-bytes) (print-string summary) (print-string "\n") (exit-success))) (let [src (read-file file-path) program (parse-program src) ir (lower program) wasm-size (emit-wasm-with-target ir opts) summary (wasm-size-text wasm-size)] (do (write-file output-path summary) (print-string summary) (print-string "\n") (exit-success)))) (exit-compile-error)))
