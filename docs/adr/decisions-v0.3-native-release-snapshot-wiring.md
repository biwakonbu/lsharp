...
へ分離して verified partial とした。native-official multi-target の offline propagation も
...
へ分離した。provider API/authentication、Mac/Linux runtime の snapshot propagation は未完了で、
`EC-M3-05` は `[~]` のまま残す。
