...
- `cargo test -p lsharp-wasm --test e2e selfhost_evidence_registry -- --nocapture`
  （13 passed）、`selfhost_intent_source_adapter`（既存 8 passed）。負の seed / shrink 値、malformed
  coverage entry、extra payload field、重複 coverage bucket、duplicate evidence ID は同じ selfhost registry error contract として確認した。
...
- selfhost parserのrequired fieldsとsamplingを含む初期 `:evidence` form、および
  ...
  負の seed / shrink 値と malformed coverage entry は `invalid-sampling` code `11` / field `seed`・`shrinks`・`coverage`、17-field 以外の payload は `malformed` code `1` / field `form` として fail-closed に固定したが、
