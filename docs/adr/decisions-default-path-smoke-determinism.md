...
- **job が skipped だった理由は、本 ADR の範囲外として残していたが 2026-08-18 に判明した。**
  job 側の条件ではなく `6651109b` (2026-07-12) が `ci.yml` から `push` / `pull_request`
  トリガを外していたためで、**workflow 全体が起動していなかった**。
  ブランチを push しても run は 0 件になることを実測で確認している。
  これにより上の「push 後の 1 run」という検証計画自体が成立しないことも確定した
  (確認には `workflow_dispatch` の手動起動が要る)。詳細は `I-15` と `I-19` が正本。
