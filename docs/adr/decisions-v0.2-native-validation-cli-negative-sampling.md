...
- Date: 2026-07-29
- Scope: selfhost `App.Cli` の `validate --source` source/report boundary
...
- 既存の `App.Cli` source graph error path を契約テストで固定し、sampling の意味論、count と
  `cases` の関係、current-source stage0 artifact/runtime parity はこの slice の対象外とする。
...
これは `App.Cli` の Rust-host actual Wasm に限定した verified partial slice である。EmbeddedCli、
current-source packaged stage0、Mac Apple Silicon / Linux x86_64 の artifact/runtime matrix、
sampling generator の実行意味論、`lsharp validate` 全体の完了を意味しない。
