...
- Date: 2026-07-28
- Scope: `selfhost/src/Tools/Validation/IntentSource.ls`, `scripts/ci/native-selfhost-dev-source-file-smoke.sh`
...
This closes whitespace semantics for source node ID/text validation only. It does not prove current-source
packaged stage0 execution, Mac/Linux artifact/runtime parity, manifest bytes, or fallback exclusion. Keep
`EC-M2-01` and M2/M3 aggregate `[~]` until actual stage0 replay covers the same fixture.
