...
- `ReviewTrustKey` は provider、key ID、algorithm、32-byte Ed25519 public key だけを持つ。
...
- wire の trust store は allowlist input の projection であり、signature verification、key
  rotation、provider fetch、trust root の署名検証は後続 boundary とする。
