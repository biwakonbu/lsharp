...
runtime helper / Linux runtime trampoline / real ELF object/link artifact は固定済みであり、actual Linux native self-regeneration も固定済みである。`stage23-map-insert-staged-merge-full-compare-v1` は全 2,897 functions の stage2/stage3 transport と materialize を完走し、stage2/stage3 stdout byte-for-byte 一致、code length 9,528,579、同一 SHA-256 `9196a2e5ef859ddcfa93d8b898282f0b0b18d2d0ac9071a4a3e9e311ed7f4ff9`、両 stderr 0、summary status `pass` を確認した。release input と同じ selfhost source tree の `8dd37ef-static-string-fixedpoint` も stage2/stage3 SHA-256 `58f8f634ee1e53d53204db20a84b4695fb37f8317915ccd217fc43fab15fc60d`、code length 9,610,828、stderr 0 で pass した。V2-15 は両 supported target の actual `App.Cli` bundle、実在 rollback archive、`native-official-release-local.sh` の Mac/Lima archive/download smoke まで完了した。native-only archive の boot smoke は実証済み `--version` / `--help` に限定し、host launcher archive の包括 command smoke は維持する。actual replay は引き続き Mac Apple Silicon 上の Lima VM local gate とし、required CI には含めない。
