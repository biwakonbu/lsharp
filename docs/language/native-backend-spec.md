...
| `x86_64-unknown-linux-gnu` | Linux x86_64。server priority track で先行検証中 | actual Linux native self-regeneration / publish wiring |
...
runtime helper / Linux runtime trampoline / real ELF object/link artifact は固定済みであり、actual Linux native self-regeneration も固定済みである。`stage23-map-insert-staged-merge-full-compare-v1` は全 2,897 functions の stage2/stage3 transport と materialize を完走し、stage2/stage3 stdout byte-for-byte 一致、code length 9,528,579、同一 SHA-256 `9196a2e5ef859ddcfa93d8b898282f0b0b18d2d0ac9071a4a3e9e311ed7f4ff9`、両 stderr 0、summary status `pass` を確認した。actual replay は引き続き Mac Apple Silicon 上の Lima VM local gate とし、required CI には含めない。Linux x86_64 の残件は V2-15 の実 `App.Cli` native release input / smoke / publish wiring である。
