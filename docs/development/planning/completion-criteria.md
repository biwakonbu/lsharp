...
- **監査整理 / bootstrap**: stage0 による selfhost 再コンパイルと `test_e2e_bootstrap_fixed_point_stage2_stage3` による `Main.ls` の true fixed point は提示済み。**ただし fixed input set 54 件側は未達である。** `test_e2e_bootstrap_stage2_self_feed_fixed_input_set` [赤: `I-78`] と `test_e2e_bootstrap_fixed_input_set_stage_chain_match` [赤: `I-78`] は `docs/development/validation/ignored-lane-expected-failures.txt` に期待 FAIL として載っており、`src/App/Cli.ls` の self-feed compile が `integer divide by zero` で trap する。**BOOT-04 完了証跡は full input set compare まで到達していない** (訂正 2026-08-28 / `I-104`)。
...
### ゲート 1: Wasm bootstrap fixed-point [in-progress]
...
- **現況**: `scripts/ci/compile-phase11-inputs.sh` は selfhost/stdlib/examples の fixed input set compile gate を通し、`RUN_BOOTSTRAP_FIXED_POINT=1` では `test_e2e_bootstrap_fixed_point_stage2_stage3`・`test_e2e_bootstrap_stage2_self_feed_fixed_input_set` [赤: `I-78`]・`test_e2e_bootstrap_fixed_input_set_stage_chain_match` [赤: `I-78`] を exact 実行する (既定は `RUN_BOOTSTRAP_FIXED_POINT=0` なので、既定の CI 経路はこの 3 本を実行しない)。`test_e2e_bootstrap_cli_fixed_input_compile_gate` で `App/Cli.ls` direct compile gate も固定され、historical compiler-mode memory.grow blocker は再現しない。ローカル再実行では script 全体が exit 0 となり、self-feed gate は fixed input set 54 件（selfhost 40 / stdlib 11 / examples 3）の stage2 self-feed 決定性を、stage-chain gate は同じ 54 件の `stage1 -> stage2` 出力と `stage2 -> stage3` 出力の bit-identical compare を `ci-artifacts/bootstrap-diff/{sha}/fixed-input-set-self-feed-report.txt` / `fixed-input-set-self-feed.json` / `fixed-input-set-stage-chain-report.txt` / `fixed-input-set-stage-chain.json` に保存する
- **未達 (訂正 2026-08-28)**: full input set に対する `stage1 -> stage2 -> stage3` の実体生成・比較は**通っていない。** 上記 2 本は `I-78` (状態 `open`) により赤である。`src/App/Cli.ls` は 54 件の fixed input set の構成員なので、「full input set の compare」という条件文は満たされていない。
  **訂正前は `[done]` / 「Gate 1 を close した」と書かれていた。**赤が台帳へ入った後も本 file を見に行く経路が無かったのが原因で、再発検出を `scripts/audit_docs.sh` へ足した。裁定は `docs/adr/decisions-completion-criteria-red-citation.md`、issue は `I-104`。
  なお artifact 生成 (`ci-artifacts/bootstrap-diff/{sha}/...`) と `test_e2e_bootstrap_fixed_point_stage2_stage3` による `Main.ls` の true fixed point は本訂正の対象外であり、否定していない
