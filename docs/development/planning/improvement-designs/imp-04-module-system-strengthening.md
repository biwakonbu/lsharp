...
Phase C-2h として、明示 root の `ArtifactCache` を追加した。artifact schema、compile key schema、key fingerprint、payload
fingerprint を envelope に含め、既存の atomic Wasm writer で保存する。file 不在や envelope/payload の不一致は cache miss
へ戻し、破損 bytes を成功扱いしない。既定 compile と `CompileSession` への自動接続、target/runtime validation、cache
eviction、native/selfhost persistence は後続の C-2i 以降で行う。

Phase C-2i として、`CompileSession::with_artifact_cache(root)` を追加し、明示 root の artifact cache を compile pipeline の
Wasm target（Preview1 / Component / WebWasm / WasmGC）へ opt-in 接続した。cache hit は output path へ atomic copy して IR
compile を省略し、miss は既存 pipeline の codegen 後に envelope を保存する。Native target、`emit_ir`、既定の
`CompileSession::new` は cache を使わず、source fingerprint 変更では fresh compile へ戻ることを cross-session test で固定した。
artifact runtime validation、CLI flag/env、eviction、selfhost/native persistence は未完了である。

Phase C-2j として、`lsharp-wasm::validation` に core Wasm / WasmGC / component の target-aware validation を追加し、
`CompileSession` の opt-in cache hit と store 前に適用した。envelope の checksum が一致しても Wasm として不正な payload は
miss として fresh compile へ戻し、生成 bytes が検証できない場合は cache/output を成功扱いしない。Native、CLI config、
runtime execution、selfhost/native stage0 parity は後続である。

Phase C-2k として、`compile` / `build` に `--artifact-cache-dir <path>` を追加した。指定時だけ Rust host が
`CompileSession::with_artifact_cache` を使い、未指定時は cache directory を作らない。embedded component delegation はこの flag
を host-only boundary として拒否し、relative path は current working directory 基準で扱う。CLI の default cache location、外部
`LSHARP_PATH` compiler、eviction、Native persistence は後続である。

Phase C-2l として、`CompileArtifacts::from_cache` を追加し、artifact hit/miss を observable にした。cross-session の warm
Wasm bytes を `wasi_runner::run_wasm_wasi` で実行し、cold/hit は `7\n`、source change の fresh compile は `8\n` を返すことを
固定した。cache hit の structural validation だけでなく runtime execution evidence までを Rust host lane で閉じたが、native
stage0、Linux x86_64、component world execution、selfhost persistence は未完了である。

Phase C-2m として、明示 root の `ArtifactCache::trim_to_entries(max_entries)` を追加した。opaque な SHA-256 key に mtime/LRU の
意味を仮定せず、`.artifact` を fingerprint の辞書順で deterministic に上限まで残し、schema 外の file と `.artifact` 以外の
...
Phase C-2o として、`compile` / `build` に `--artifact-cache-max-bytes <N>` を追加し、entry limit と同じ明示 root / successful compile
後の host maintenance 境界へ接続した。`ArtifactCache::trim_to_bytes` は envelope file size の合計を計測し、辞書順最小の artifact
