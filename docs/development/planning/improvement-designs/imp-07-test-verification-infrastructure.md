...
- GC soak telemetry lane: `scripts/ci/test-gc-soak-telemetry.sh --profile verified` が unrooted reclaim / rooted reuse / legacy sentinel の 3 exact E2E を実行する。`--profile soak` は ignored の in-session REPL / actual `lsp --stdio` telemetry 2 件を明示実行するが、現状は stack overflow または `root_set` (Wasm function 24) trap で RED となるため、verified slice と blocker を分離して記録する。host/Wasmtime stack を 128 MiB に拡大しても REPL exact test は 452.67 秒後に同じ root slot boundary で失敗し、次の RED は compiler-side root slot ledger / safe-point spill の観測に限定する。
...
これらは arbitrary input/type の panic regression、unification の成否対称性、深い型注釈・巨大レコードの限界値、GC leak / free-list の actual runtime、限定した AST roundtrip、固定 seed の 4096-case wide lane に対する verified slice であり、
小さな式全体の型推論の full-program parity、性能回帰の閾値・issue 化、runtime memory.grow の HTTP/component/native parity、rooting stress/static lint、native stage0 の GC gate は未着手のまま残る。property test と benchmark は dev-dependency に閉じ、
