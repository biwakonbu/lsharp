...
これは no-arg / 1-arg〜61-arg / generic 20+-arg helper path / intra-bundle の call-frame discipline を示す partial evidence であり、representative stage23 の `Call`（import/runtime 境界、representative stage23 integration、実 LoweredModule function plumbing）をまだ置き換えてはいない。
その結果 `actual-stage23-gap.json` の先頭 blocker は引き続き representative `Call`、`Drop`、integer logic/data ops（`I32And/Or`, `I32Load(8U)/Store`, `I64Load/Store`, `I64Mul/Div/Rem`）、および control-flow / memory ops（`BlockEmpty`, `Br`, `BrIf`, `Else`, `End`, `If`, `IfEmpty`, `Loop`, `LoopEmpty`, `MemoryCopy`, `MemoryFill`）である。主要 i64 compare (`I64Eq`, `I64Ne`, `I64LtS`, `I64GtS`, `I64LeS`, `I64GeS`) は x86_64 / aarch64 の host exact と x86 compare tail bytes で固定し、representative gap report からも外れた。
