...
その結果 `actual-stage23-gap.json` の先頭 blocker は引き続き representative `Call`、`Drop`、integer data ops（`I32Load(8U)/Store`, `I64Load/Store`, `I64Mul/Div/Rem`）、および control-flow / memory ops（`BlockEmpty`, `Br`, `BrIf`, `Else`, `End`, `If`, `IfEmpty`, `Loop`, `LoopEmpty`, `MemoryCopy`, `MemoryFill`）である。主要 i64 compare (`I64Eq`, `I64Ne`, `I64LtS`, `I64GtS`, `I64LeS`, `I64GeS`) と `I32And` / `I32Or` は x86_64 / aarch64 の host exact と x86 exact bytes で固定し、representative gap report からも外れた。
