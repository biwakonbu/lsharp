...
- `LSHARP_PATH=<*.component.wasm>` の `compile` / `build` は guest summary を残しつつ host launcher 側で実 artifact を書ける。また `LSHARP_PATH` 未設定時でも、`lsharp` 実行ファイルの隣に `lsharp.component.wasm` があれば build-time embedded guest よりその sidecar を優先する。
