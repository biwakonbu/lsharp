...
Evidence: `test_e2e_selfhost_test_runner_rejects_non_bool_unselected_match_arm`（`1 passed`、26.12s）、`test_e2e_selfhost_test_runner_rejects_non_bool_match_guard`（`1 passed`、28.95s、guard式の exact span 一致）、`test_e2e_selfhost_test_runner_rejects_non_bool_compound_match_guard_span`（`1 passed`、28.99s、`if` guard 全体の exact span 一致）、`test_e2e_selfhost_test_runner_rejects_non_bool_function_match_guard`（`1 passed`、26.74s、user-defined function の静的 body shape と guard call の exact span 一致）、既存 strict Bool 5件（`5 passed`、425.54s）、valid guarded-match differential（`1 passed`、28.99s）、`legacy_invariant_match_guard_requires_bool` と `metadata_contract_check` 30件（全件 pass）。これは Rust metadata checker と Rust-host Wasm selfhost runnerの match-arm/guard code、静的 non-Bool guard span、静的に解決できる user-defined guard の verified sliceに限定され、guardの exact message/report forwarding、未知・動的 guard の全 span parity、Mac Apple Silicon / Linux x86_64 current-source native artifact/runtime、EC-M1-01 aggregate、他の未分類 higher-order/type-directed expression parityの完了を意味しない。TODOの `[~]` と Rust oracle / bootstrap / host integration境界は維持する。
