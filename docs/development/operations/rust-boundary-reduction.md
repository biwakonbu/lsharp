...
- Linux native fixed point for `45fca123`: `ci-artifacts/native-linux-x86-hostgen-vm/45fca123-curried-fun-batch/actual-selfregen-summary.json` は `status=pass`、stage2/stage3 code length は各 `11421747`、stdout SHA-256 は両方 `2f6eeb64020c472c7ebfce4a6a208b1ed34eeb83e0b954e005cb545593dffa29`、stage1 manifest の `source_commit` は `45fca12338982e711ccbfc35fa97bda294764461`、`function_start_len` は `3438`、`main_func_idx` は `3447`。stage2/stage3 stdout は byte-for-byteで一致し、stderr は各 0 bytes。`TRANSPORT_CHUNK_SIZE=128` / `ACTUAL_CHUNK_SIZE=128` で split/retryなしに完走し、検証後は stage1/2/3 debug bundle と VM replay lock/workdirを回収して約 `28M` artifactへ縮小した。current-source stage0 package、Mac Apple Silicon current-source gate、full source-file/public command parity、signature collection/annotation loops、V2-16 aggregateは残件である。
