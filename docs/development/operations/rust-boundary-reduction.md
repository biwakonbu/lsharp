...
actual compiler-mode/ftable record-pattern regressionは `RUST_MIN_STACK=33554432` で実行しても `load-imports-from-decls-step` instruction 78の root ledger `BranchDepthMismatch` (`then_depth=6`, `else_depth=7`) に到達した。変更なしの `origin/main` baselineでも同じ failure valueを再現したため、これは今回の parser/TypeInferPattern差分起因ではない既存 blockerとして分離する。この sliceは qualified record patternの parser/type-inference境界だけを閉じ、constructor pattern、private record、record update、standalone native stage0、Wasm artifact/runtime、Mac Apple Silicon / Linux x86_64 のこの変更後 current-source native gate、EC-M1-01 aggregateの完了を意味しない。`TODO.md` の `[~]` と Rust oracle / bootstrap / host integration境界を維持する。次は別の qualified pattern/update contractを一つの REDに固定する。
...
### EC-M1-01 selfhost imported record update compiler-mode blocker (2026-07-25)

actual Wasm runtimeの最小 fixtureとして `App.Shapes.Point` を `App.Main` から alias + `:only [Point]` で importし、`(S.Point 40 2)` を `{point | x 41}` へ updateして `(. updated x)` / `(. updated y)` を出力する testを追加した。`test_e2e_selfhost_compiler_mode_imported_record_update_runs` は `RUST_MIN_STACK=33554432` で 70.5秒後に `load-imports-from-decls-step` instruction 78の root ledger `BranchDepthMismatch` (`then_depth=6`, `else_depth=7`) で失敗し、Wasm runtimeには到達しなかった。

同じ failure valueは変更なし `origin/main` の record-pattern compiler-mode baselineでも再現済みであり、qualified record updateの parser/type-inference差分へ原因を帰属させない。この testは RED contractとして残し、root ledgerを修正した後に actual source/ftable runtime、artifact bytes、Mac Apple Silicon / Linux x86_64 native gateを再検証する。root lifetime修正、ftable import update、record update runtime parity、standalone native stage0、Wasm artifact/runtime、EC-M1-01 aggregateの完了を意味せず、`TODO.md` の `[~]` と Rust oracle / bootstrap / host integration境界を維持する。
