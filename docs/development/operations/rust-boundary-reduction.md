...
- RED/GREEN: `test_e2e_selfhost_parser_record_decl_fields_use_bounded_chunks`、`test_e2e_selfhost_parser_record_decl_fields_cross_chunk_boundary`、`test_e2e_selfhost_parser_type_variants_use_bounded_chunks`、`test_e2e_selfhost_parser_type_variants_cross_chunk_boundary`、`test_e2e_selfhost_parser_type_alias_params_use_bounded_chunks`、`test_e2e_selfhost_parser_type_alias_params_cross_chunk_boundary`。65-field record declaration、65 variants、最初の variant の 65 field types、65 type-alias parameters を selfhost runtime で確認した。
- Focused checks: record parser 10 tests、lexer keyword consistency 1 test、type-expression parser 1 test、`cargo run --quiet --bin lsharp -- parse selfhost/src/Syntax/Parser.ls` (`decls:309`, `diagnostics:0`)。type-alias parameter slice の既存 parametric type alias 2 tests と ADT type variant 2 tests も通過した。
