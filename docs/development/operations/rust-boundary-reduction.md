...
Evidence: Rust `test_run_metadata_tests_executes_deterministic_property_smoke` / `test_run_metadata_tests_reports_failing_deterministic_property` / `test_run_metadata_tests_rejects_property_outside_deterministic_smoke_profile`、selfhost `test_e2e_selfhost_runner_executes_deterministic_property_smoke`、`test_e2e_selfhost_cli_reports_deterministic_property_smoke`、`test_e2e_selfhost_cli_rejects_non_bool_deterministic_property`、Rust `metadata_check` 29 tests、Wasm `test_runner` unit 6 tests。Rust 側は `PropertySmokeTestSpec` と固定 sample projection、selfhost 側は `Tools.Test.PropertyRunner` の raw projection と既存 evaluator を使う。
