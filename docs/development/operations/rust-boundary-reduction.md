...
- selfhost compiler IR と native codegen には `write-file` の dedicated opcode/runtime helper がない。native `program.native` は source-file output を単独で提供できない。
...
1. `V2-16b`: byte-output ABI と両 target の native `write-file` runtime helper を追加し、`App.Cli` の compile/build が actual Wasm bytes を出力するようにする。
