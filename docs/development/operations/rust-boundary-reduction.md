...
- selfhost Wasm の standalone Preview1 first slice は、標準 `wasi_snapshot_preview1.fd_write` only import、record/accessor output、root_push/root_set/root_pop output、static string literal の `print-string` output を `run_wasm_wasi` で検証済みである。未実装の `string-concat` などは compile error として artifact を生成しない。これは全 capability parity ではなく、runtime stub の誤成功を防ぐための最小 boundary であり、string concat/substring、ファイル/argv runtime、dynamic root stack capacity、動的 data/heap layout、component sidecar は残件である。
