...
### EC-M1-03 canonical case failure message slice (2026-07-22)

selfhost `TestRunner` の canonical `:case` について、期待値不一致時の Rust oracle message を source span から再構築し、result の末尾へ保持する経路を追加した。`generate-tests` は既存の `run-cases` 評価経路を維持したまま、評価後の別 pass で `passed=0` かつ diagnostic code がない case resultだけを message 付き resultへ置換する。message生成中は source、case、message を root stack で保持し、source-less `run-cases` の結果形状と空 message境界は変更しない。
...
### EC-M1-06 mixed `:case` / `:property` structured report parity (2026-07-22)

同一 `defn` に canonical `:case` 1 件と sampled `:property` (`:cases 2`) を置いた fixtureで、Rust oracle と selfhost の report semantics を分離して固定した。Rust oracle の `TestResult` は property を論理テスト 1 件として返すため、oracle は case 1 件 + property 1 件の 2 logical results となる。一方、公開 `test --format json` report の `cases` / `coverage.executed` は実サンプル件数を表すため、Cli と EmbeddedCli は case 1 + property samples 2 = 3 と報告する。
...
### EC-M1-06 App.Cli text runtime failure coverage (2026-07-23)

App.Cli の text assurance が EmbeddedCli と同じく runtime failure を正しく表現することを検証した。canonical `:case` 2 件のうち 1 件が失敗する fixtureで、text report は `status=fail`、`method=explicit-case`、`contracts=2`、`cases=2`、`coverage.executed=2`、`coverage.failed=1`、`diagnostics.count=0`、exit 2 を返す。report は 28 行で、top-level `verified` を出さない。入口固有値は `runner=selfhost-cli`、`target=runtime-selected` とした。
...
### EC-M1-06 JSON assurance provenance shape parity (2026-07-23)

ADR-192 で入口固有の runner/target を JSON へ反映した後も、text report の `producer`、`tool_version`、`source_digest`、`timestamp` が JSON では欠落していた。Cli と EmbeddedCli の `assurance-provenance-json` helper に同じ4フィールドを追加し、`producer=lsharp-selfhost`、`tool_version=0.1.0`、未注入の `source_digest`/`timestamp=unknown` を固定した。既存の `source_commit`/`artifact_digest=unknown` と entry-specific runner/target は変更していない。
...
### EC-M1-06 App.Cli mixed JSON failure provenance (2026-07-23)

ADR-197 で EmbeddedCli mixed failure の entry-specific provenance を閉じたため、対になる App.Cli の実 argv failure pathにも同じ JSON contractを追加した。実装側の report schemaは変更せず、canonical `:case` failure 1件と sampled `:property` success 2 samplesを App.Cli runtime-selected entrypointで実行し、既存の mixed runtime failure aggregation と provenance shapeを同時に検証する。
...
### EC-M1-06 all-form text assurance runtime failure (2026-07-23)

ADR-199 で全 metadata formの JSON execution accountingを閉じたため、同じ境界を text runtime failureでも actual executionへ拡張した。fixtureは `:example`、`:invariant`、`:assert`、2件の `:case`（1件失敗）、3 sampleの `:property` を含み、Rust oracleの logical result 6件・失敗1件と selfhostの実行件数8件を意図的に分離する。
