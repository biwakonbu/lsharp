...
### 日常開発を Rust なしで開始する判定 (2026-07-16)

Mac Apple Silicon (`aarch64-apple-darwin`) と Linux x86_64 (`x86_64-unknown-linux-gnu`) の verified stage0 package があり、source fingerprint が一致する場合は、`scripts/native-selfhost-dev.sh` の `parse` / `check` / `fmt` / `test` / `compile -o` / `build -o` を Rust toolchain と host `lsharp` なしで日常開発に使ってよい。したがって、L# の対応済み core slice は L# で実際に開発しながら自己ホスト側へ置き換えを進める運用へ移行する。
