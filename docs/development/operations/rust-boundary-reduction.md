...
Evidence: `test_e2e_selfhost_parser_typed_property_contract_preserves_expression_spans` が同一 fixtureで binder `40..49`、precondition `75..87`、postcondition `104..120`を検証し、focused E2Eは `1 passed`（24.35s）だった。current source commit `9e8dab64fe54fb37a720695e0c2e8b38019df27d` の Linux x86_64 gateも `status=pass`、stage2/stage3 stdout SHA-256 `2ae6f1406e5c0484a94282a17edac85fad9f3f5649352c43db1826979a576ed6` 一致、code length `10,832,651` bytes 一致、stage1 manifestの `source_commit` 一致を確認した。VMは検証後に停止し、12 GiB disk / 16 GiB RAM構成を維持した。

これは typed contract inventoryの source-span verified sliceに限定される。source-aware contractを既存の source-less property test-case materializerへ直接流し込むと、legacy heterogeneous vector inferenceが `expected Int, found String` で止まるため、公開 `generate-tests` runtime pathは今回変更せず、Rust-free runtime parityの証拠へ拡張していない。runtime test-case span propagation、predicate/source diagnostic parity、Mac Apple Silicon current-source gate、EC-M1-02 aggregate、Rust oracle / bootstrap / host integration境界は残件である。したがって TODOの `[~]` と明示的な Rust boundary は維持する。
