...
> - E2E: `crates/lsharp-wasm/tests/e2e.rs` に `#[test]` **471 件**（通常実行は **470 passed / 1 ignored**）。ブートストラップ検証の主経路は `try_compile_and_run_file` / `compile_and_run_file`（マルチファイル・import）。インラインソース用の `try_compile_and_run` は将来の最小再現テスト用に **残置**（現状 `#[allow(dead_code)]`）。
...
- [~] [`CLI-02 13 command implementations`](docs/development/planning/phase11-implementation-plan.md#cli-02-13-command-implementations) -- `selfhost/Cli.ls` では `show-help` / `show-version`、文字列ベース `arg-parse`、parse/check/test/review/fmt/compile/doc の in-memory core と file-path handler、さらに `run-doc-ack` / `run-doc-check` / `run-install` / `run-repl` / `run-lsp` までは TDD で接続済み。加えて `parse-diagnostics-count` / `check-diagnostics-count` により parse/check の構造化エラー返却基盤を追加（`test_e2e_selfhost_cli_parse_diagnostics`, `test_e2e_selfhost_cli_check_diagnostics`）。stdout/stderr・各コマンドの deeper contract parity は未実装。
- [~] [`LSP-02 10 method parity`](docs/development/planning/phase11-implementation-plan.md#lsp-02-10-method-parity) / [`LSP-03 Diagnostic ordering`](docs/development/planning/phase11-implementation-plan.md#lsp-03-diagnostic-ordering-and-json-snapshots) -- `selfhost/LspServer.ls` では initialize/didOpen/didChange/formatting/completion と navigation/diagnostic sort+merge の runtime-observable PoC に加え、JsonRpc 契約と揃えた method hash dispatch と 1-message `server-loop` PoC を備え、さらに `sort-diagnostics` を N 要素対応の挿入ソートに汎化（`test_e2e_selfhost_lsp_sort_diagnostics_three`, `test_e2e_selfhost_lsp_sort_diagnostics_four`）。JSON-RPC transport / snapshot parity は未実装。
- [~] [`FMT-01 Formatter roundtrip`](docs/development/planning/phase11-implementation-plan.md#fmt-01-formatter-roundtrip) / [`DOC-01 Schemas and snapshots`](docs/development/planning/phase11-implementation-plan.md#doc-01-schemas-and-snapshots) -- `format-program` は依然 vector-length ベースだが、`format-expr` を lit-int/var に加え apply (tag=5) / if (tag=6) に拡張（`test_e2e_selfhost_formatter_format_expr_lit_int`, `test_e2e_selfhost_formatter_format_expr_apply`）。`selfhost/DocTools.ls` では top-level / module body の public `defn` / type 系宣言抽出と deterministic doc count に加え、`generate-html` 関数を追加し HTML doc 構造 (5 スロット) の deterministic 生成と idempotency を TDD で固定（`test_e2e_selfhost_doctools_generate_html_basic`, `test_e2e_selfhost_doctools_generate_html_idempotent`）。完全 roundtrip / schema snapshot parity は未達。
...
- [~] [`GC-05 LSP soak and REPL GC`](docs/development/planning/phase11-implementation-plan.md#gc-05-lsp-soak-and-repl-gc) -- 縮小版 `test_e2e_gc_light_compile_run_loop`（48 回 compile+run）に加え、`test_e2e_gc_compile_run_loop_1000`（1,000 回、`#[ignore]`）を追加。500 eval REPL は未達。
- [~] [`GC-06 Leak detection and metrics`](docs/development/planning/phase11-implementation-plan.md#gc-06-leak-detection-and-metrics) -- `__alloc` の bump allocator メトリクス観測基盤を追加。`test_e2e_alloc_metrics_peak_usage`（3 alloc のアドレス単調増加+スパン検証）、`test_e2e_alloc_metrics_monotonic_check`（100 alloc の再帰ループでアドレス単調増加検証）。heap RSS / GC pause の CI gate 化は未達。
...
- [~] `CP-04 Public toolchain` -- `selfhost/Cli.ls` は 13 サブコマンド全てに observable PoC を備え、`parse-diagnostics-count` / `check-diagnostics-count` で構造化エラー返却基盤を追加。`selfhost/LspServer.ls` は `sort-diagnostics` を N 要素対応の挿入ソートに汎化し、3/4 要素ソートテストを追加。`selfhost/Formatter.ls` は `format-expr` を apply/if にも拡張。`selfhost/DocTools.ls` は `generate-html` で deterministic HTML doc 構造を生成。deeper JSON-RPC transport parity / 完全 formatter roundtrip / schema snapshot parity は未完。Evidence: `selfhost/Cli.ls`, `selfhost/DocTools.ls`, `selfhost/LspServer.ls`, `selfhost/Formatter.ls`, `selfhost/TestRunner.ls`, `crates/lsharp-wasm/tests/e2e.rs`（`test_e2e_selfhost_cli_parse_diagnostics`, `test_e2e_selfhost_cli_check_diagnostics`, `test_e2e_selfhost_lsp_sort_diagnostics_three`, `test_e2e_selfhost_lsp_sort_diagnostics_four`, `test_e2e_selfhost_formatter_format_expr_lit_int`, `test_e2e_selfhost_formatter_format_expr_apply`, `test_e2e_selfhost_doctools_generate_html_basic`, `test_e2e_selfhost_doctools_generate_html_idempotent`）
- [~] `CP-05 Runtime stability` -- `test_e2e_gc_light_compile_run_loop`（48 回）に加え `test_e2e_gc_compile_run_loop_1000`（1,000 回、`#[ignore]`）を追加。`__alloc` bump allocator のメトリクス観測基盤として `test_e2e_alloc_metrics_peak_usage`（アドレス単調増加+スパン検証）、`test_e2e_alloc_metrics_monotonic_check`（100 alloc 再帰ループ）を追加。heap RSS / GC pause の CI gate 化は未達。Evidence: `crates/lsharp-wasm/tests/e2e.rs`
...
- [~] [CLI-02 13 command implementations](docs/development/planning/phase11-implementation-plan.md#cli-02-13-command-implementations) -- `selfhost/Cli.ls` は 13 サブコマンドすべてに observable PoC を持ち、`parse-diagnostics-count` / `check-diagnostics-count` で構造化エラー返却基盤を追加。stdout/stderr・各コマンドの deeper contract parity は未実装。
- [~] [LSP-02 10 method parity](docs/development/planning/phase11-implementation-plan.md#lsp-02-10-method-parity) -- `selfhost/LspServer.ls` は runtime-observable PoC に加え、`sort-diagnostics` を N 要素対応の挿入ソートに汎化（3/4 要素テスト追加）。JSON-RPC transport / snapshot parity は未実装。
- [~] [FMT-01 Formatter roundtrip](docs/development/planning/phase11-implementation-plan.md#fmt-01-formatter-roundtrip) -- `format-program` は `vector-length` ベース。`format-expr` は lit-int/var に加え apply (tag=5) / if (tag=6) に拡張。完全 roundtrip は未。
- [~] [DOC-01 Schemas and snapshots](docs/development/planning/phase11-implementation-plan.md#doc-01-schemas-and-snapshots) -- `selfhost/DocTools.ls` に `generate-html` 関数追加。HTML doc 構造 (5 スロット) の deterministic 生成と idempotency をテストで固定。完全 schema snapshot parity は未達。
- [~] [GC-05 LSP soak and REPL GC](docs/development/planning/phase11-implementation-plan.md#gc-05-lsp-soak-and-repl-gc) -- `test_e2e_gc_light_compile_run_loop`（48 回）+ `test_e2e_gc_compile_run_loop_1000`（1,000 回、`#[ignore]`）。500 eval REPL は未。
- [~] [GC-06 Leak detection and metrics](docs/development/planning/phase11-implementation-plan.md#gc-06-leak-detection-and-metrics) -- `__alloc` bump allocator のメトリクス観測基盤追加（`test_e2e_alloc_metrics_peak_usage`, `test_e2e_alloc_metrics_monotonic_check`）。heap RSS / GC pause の CI gate 化は未達。
