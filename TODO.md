...
- [ ] `NATIVE-DEAD-01` 呼び出し元 0 の native emitter helper の裁定 — Issue `I-25`。
  ...
  **受入条件**: (1) chunk1-4 と本体のバイト列突き合わせ — **済 (2026-08-19)**。
  長さは 77 word で一致するが `0x34` 以降 63 word が乖離し、しかも chunk 群の方が
  後発 (`e9f761cb` 2026-04-29 > `901c10d8` 2026-04-22)。差は bit 63 のタグ判定
  (`tbz`) の有無と、長さ 0 の default 経路の有無 (詳細は `I-25`)。
...
- [ ] `DOC-09-01` `0bd8bd47` が `TODO.md` から削除した 1,364 行を棚卸しし、救出すべき記述を
