...
- [~] `LEGACY-MAINT-01` large-file decomposition — Issues `I-01` / `I-08`。多数の test/
  ...
  `Module` / `Function` / GC model、linker seam、`validation_source` node/evidence/typed edge seam、compile surface seam、validation input manifest/reference seam、native emitter memory seam、validation output manifest wire seam は verified。`wasi.rs`、`lsharp-ir/src/lib.rs`、`lsharp-tooling/src/compile.rs`、
