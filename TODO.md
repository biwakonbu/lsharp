...
- [~] Step 2 exit gate -- 4 層比較 + ステージチェーン検証に加え、minimal subset / extended `do` / zero-arg multi-fn call / single-param call / let local / 5 defn loop / memory-only builtin (`string-char-at` / `string-length` / `vector-length` / `vector-get`)、`vector-*` / `ref-*` + `env.__alloc` import、integer-key `map-new` / `map-insert` / `map-get` / `map-size` / `map-contains?` / `map-remove`、source-aware literal string-key subset、`env.__alloc` + `env.print` import を使う `print`、host mock が exported memory へ String object を書き込む dummy-path `read-file`、さらに direct + nested `do` string literal の data section slice までは real stage1→stage2 wasm 生成を確認済み。`docs/development/operations/bootstrap-diff-artifacts.md` の CI diff 仕様追加も済んだ。**ただし** full input set の stage1→stage2→stage3 実体生成には actual path string を伴う `read-file` semantics、`do` を超えるネスト string literal の一般化、alloc-only / alloc+print+read-file を超える一般化された import-helper lowering が不足しており、selfhost map 側も general string-key hashing runtime parity / non-literal string-key map surface は未了で、self-bootstrap fixed-point gate は未了。
...
- [~] `CP-01 Frontend/bootstrap` -- Step 1 は完了し、WASM-03 / oracle 4 連一致 / 4 層比較 / ステージチェーン検証で stage0 proxy evidence は揃った。さらに minimal subset / extended `do` / zero-arg multi-fn call / single-param call / let local / 5 defn loop に加え、memory-only builtin (`string-char-at` / `string-length` / `vector-length` / `vector-get`) と `vector-*` / `ref-*` + `env.__alloc` import、integer-key `map-new` / `map-insert` / `map-get` / `map-size` / `map-contains?` / `map-remove` の real stage1→stage2 wasm 生成 slice まで確認済みで、`Compiler.ls` / `WasmEmit.ls` も per-function metadata ベースの multi-function + signature/local emission、memory builtin opcode、narrow alloc-import helper、`memory.copy` を含む vector growth emission、ref cell emission、fixed-capacity map emission へ前進した。**ただし** `print` / `read-file` を含む compiler bundle 全体を stage2→stage3 へ self-feed する lowering/import 層は未実装で、selfhost map 側も string-key hashing / string-key map surface は未了のため、fixed point gate は reopen。Evidence: `crates/lsharp-wasm/tests/e2e/selfhost_bootstrap_four_layer.rs`, `crates/lsharp-wasm/tests/e2e/strings_patterns_compiler_integration.rs`, `docs/development/operations/bootstrap-diff-artifacts.md`
...
- [ ] **C-3. パッケージ API diff & 互換性チェック**
  - [ ] `lsharp api-diff <old-version> <new-version>` コマンド: 2 バージョン間の api.json 比較
