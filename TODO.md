...
- [~] `CP-02 Syntax/types parity` -- syntax/type 系テストはさらに増えており、selfhost parser では宣言タグ正規化・追加キーワード/特殊記号 token 整合・quote/unquote 系 parser・可変長ノード count field 更新・record literal / record update parser・`defmacro` / `private` 宣言 parser・`type-alias` / `type-constrained` / `computation-builder` / `impl` の decl tag 対応・`(: expr Type)` の `ann` form parser と AST/Linter traversal・float literal の `tok-float` / parser / `ast-lit-float` 最小 payload・`computation` expression parser の step-kind / builder-hash 最小 payload・`defn` の annotated param / return type signature skip・`defn` の `:where` clause skip・`defn` metadata directive (`:doc`, `:params`, `:returns`, `:invariant`, `:transitions`) skip・`if` expression の explicit e2e・parametric `type` / `type-alias` head 対応・nested `module` / `trait` / `impl` body payload・trait method の bodyless `defn ... : Type` 最小受理・`type-constrained` の `range` / `matches` / `satisfies` coverage・multiple `:where` clauses coverage・`()` の unit literal parser まで TDD で固定済み。type 側でも selfhost `TypeInfer` に float/unit literal・`ann` form・record literal / record update・`computation` expression の direct e2e を追加し、最小型推論を固定した。`cargo build && cargo test && cargo clippy --quiet && bash scripts/audit_docs.sh` は再 green（E2E **383 passed**、docs audit **error 0 / warning 0**、clippy は既存の unrelated warning のみ）。ただし完了判定に必要な parity table は未充足。Evidence: `docs/development/planning/compatibility-matrix.md`, `docs/development/planning/completion-criteria.md`, `test_e2e_selfhost_parser_nested_module_decl`, `test_e2e_selfhost_parser_trait_impl_bodies`, `test_e2e_selfhost_parser_type_constrained_constraint_forms`, `test_e2e_selfhost_parser_defn_multiple_where_clauses`, `test_e2e_selfhost_parser_unit_literal`, `test_e2e_selfhost_parser_if_expr`, `test_e2e_selfhost_parser_parametric_type_heads`, `test_e2e_selfhost_parser_ann_form`, `test_e2e_selfhost_parser_float_literal`, `test_e2e_selfhost_parser_computation_expr`, `test_e2e_selfhost_parser_typed_defn_signature`, `test_e2e_selfhost_parser_defn_where_clause`, `test_e2e_selfhost_parser_defn_metadata_directives`, `test_e2e_selfhost_parser_defn_string_metadata`, `test_e2e_selfhost_parser_defn_params_metadata`, `test_e2e_selfhost_typeinfer_float_and_unit_literals`, `test_e2e_selfhost_typeinfer_ann_expr`, `test_e2e_selfhost_typeinfer_record_literal`, `test_e2e_selfhost_typeinfer_record_update`, `test_e2e_selfhost_typeinfer_computation_expr`
