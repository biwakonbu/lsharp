...
class NativeSelfhostInstallTest(unittest.TestCase):
    ...
    def test_promotion_failure_restores_previous_state(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = pathlib.Path(temporary_directory)
            project = root / "project"
            dependency = root / "local-lib"
            ...
            environment, marker = self.poison_host_commands(root)
            environment["LSHARP_TEST_INSTALL_FAILPOINT"] = "promotion:1"

            result = self.run_installer(project, environment)

            self.assertNotEqual(result.returncode, 0)
            self.assertIn("test-only package promotion failpoint", result.stderr)
            self.assertFalse(marker.exists(), "Cargo/lsharp fallback must not run")
    ...
    def test_sync_failure_restores_previous_state(self):
        for failpoint in (
            "promotion-before-sync",
            "promotion-after-sync",
            "lock-sync",
            "index-sync",
        ):
            with self.subTest(failpoint=failpoint), tempfile.TemporaryDirectory() as temporary_directory:
                ...
                self.assertNotEqual(result.returncode, 0)
                self.assertIn(f"test-only {failpoint} sync failpoint", result.stderr)
                self.assertFalse(marker.exists(), "Cargo/lsharp fallback must not run")
                self.assertTrue(destination.is_symlink())
                self.assertEqual(destination.resolve(), old_target.resolve())
                git_source = f"git:{repository}"
