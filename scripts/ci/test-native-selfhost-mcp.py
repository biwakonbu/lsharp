...
from native_selfhost_mcp_package_tests import assert_install_is_explicit_external_boundary, assert_package_api_generates_from_native_doc, assert_package_api_generates_from_native_doc_for_nested_source, assert_package_api_ignores_special_src_entry, assert_package_api_projects_local_api_json, assert_package_api_rejects_direct_src_file_symlink, assert_package_api_rejects_identity_mismatch, assert_package_api_rejects_invalid_arguments, assert_package_api_rejects_malformed_native_doc, assert_package_api_rejects_nested_src_symlink, assert_package_api_rejects_symlinked_api_json, assert_package_api_rejects_symlinked_src_directory, assert_package_discovery_ignores_symlink_directories, assert_package_discovery_ignores_symlinked_manifest, assert_search_projects_local_packages, assert_search_rejects_invalid_arguments
from native_selfhost_mcp_context_tests import (
    ...
    assert_project_context_rejects_invalid_arguments,
)
...
from native_selfhost_mcp_check_tests import assert_check_accepts_valid_migration_diagnostics, assert_check_projects_legacy_clean_summary, assert_check_rejects_blank_source_before_native, assert_check_rejects_invalid_arguments_before_native, assert_check_rejects_invalid_output, assert_check_rejects_legacy_unsupported_summary, assert_source_input_schema_requires_non_empty_strings
...
class NativeSelfhostMcpTest(unittest.TestCase):
        ...
        program.write_text(
            textwrap.dedent(
                ...
                if args[:1] == ["check"]:
                    ...
                    elif check_mode == "legacy-unsupported":
                        check_output = json.dumps({{
                            "command": "check",
                            "type": "Int",
                            "diagnostics": {{"count": 0, "firstErrorCode": 0, "message": ""}},
                            "migration": [{{"code": "LS2001"}}],
                            "failureKinds": [0],
                        }})
                ...
                if args[:1] == ["validate"]:
                    ...
                    if report_mode == "missing":
                        ...
                        if report_mode == "attestation-missing":
                            attestation.pop("signature")
                        elif report_mode == "attestation-extra":
                            attestation["extra"] = True
                        elif report_mode == "attestation-state":
                            attestation["state"] = "maybe"
                        elif report_mode == "attestation-bytes":
                            attestation["canonical_bytes"] = [256]
                        elif report_mode == "attestation-span":
                            attestation["span"]["start"] = -1
                        elif report_mode == "attestation-semantic-verified":
                    ...
                    if "--emit-manifest" in args:
                        ...
                        elif manifest_mode in ("receipt-valid", "receipt-missing", "receipt-mismatch"):
                            receipt_path = args[args.index("--review-verification-receipt") + 1]
                            receipt = json.loads(pathlib.Path(receipt_path).read_text(encoding="utf-8"))
                            review = {{
                                "namespace": "orders",
                                "key": "reviewer-001",
                                "provenance_digest": "sha256:receipt",
                                "visibility": "public",
                                "verification_state": "verified",
                                "verification_receipt": receipt,
                            }}
                            if manifest_mode == "receipt-missing":
                                review.pop("verification_receipt")
                            elif manifest_mode == "receipt-mismatch":
                                review["verification_receipt"] = dict(receipt)
                                review["verification_receipt"]["attestation_digest"] = "sha256:" + "c" * 64
    ...
    def test_package_api_generates_from_native_doc_for_nested_source(self):
        assert_package_api_generates_from_native_doc_for_nested_source(self)
    ...
    def test_initialize_tools_and_supported_calls_stay_native_only(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            manifest_review_schema = manifest_schema["properties"]["reviews"]["items"]
            self.assertFalse(manifest_review_schema["additionalProperties"])
            self.assertIn("verification_receipt", manifest_review_schema["properties"])
    ...
    def test_project_context_rejects_ambiguous_dependency_sources_before_native_execution(self):
        assert_project_context_rejects_ambiguous_dependency_sources(self)
    ...
    def test_check_rejects_legacy_unsupported_summary(self):
        assert_check_rejects_legacy_unsupported_summary(self)
    ...
    def test_package_api_rejects_identity_mismatch(self):
        assert_package_api_rejects_identity_mismatch(self)
    def test_package_api_rejects_symlinked_api_json(self):
        assert_package_api_rejects_symlinked_api_json(self)
    def test_package_api_rejects_symlinked_src_directory(self):
        assert_package_api_rejects_symlinked_src_directory(self)
    def test_package_api_rejects_nested_src_symlink(self):
        assert_package_api_rejects_nested_src_symlink(self)
    def test_package_api_rejects_direct_src_file_symlink(self):
        assert_package_api_rejects_direct_src_file_symlink(self)
    def test_package_api_ignores_special_src_entry(self):
        assert_package_api_ignores_special_src_entry(self)
    ...
    def test_explicit_verification_receipt_is_projected_into_manifest_review(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = pathlib.Path(temporary_directory)
            program = self.write_fake_program(root)
            receipt = root / "receipt.json"
            ...
            self.assertFalse(response["isError"])
            self.assertEqual(
                response["structuredContent"]["manifest"]["reviews"],
                [{
                    "namespace": "orders",
                    "key": "reviewer-001",
                    "provenance_digest": "sha256:receipt",
                    "visibility": "public",
                    "verification_state": "verified",
                    "verification_receipt": receipt_value,
                }],
            )

    def test_explicit_verification_receipt_manifest_projection_is_required(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = pathlib.Path(temporary_directory)
            program = self.write_fake_program(root)
            receipt = root / "receipt.json"
            ...
            result = self.run_shim(
                ...
                root,
                report_mode="receipt-valid",
                manifest_mode="receipt-missing",
            )

            self.assertEqual(result.returncode, 0, result.stderr.decode())
            response = self.responses(result.stdout)[0]["result"]
            self.assertTrue(response["isError"])
            self.assertIn("manifest receipt projection", response["content"][0]["text"])

    def test_explicit_verification_receipt_manifest_projection_mismatch_is_rejected(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = pathlib.Path(temporary_directory)
            program = self.write_fake_program(root)
            receipt = root / "receipt.json"
            ...
            result = self.run_shim(
                ...
                root,
                report_mode="receipt-valid",
                manifest_mode="receipt-mismatch",
            )
    ...
    def test_provider_snapshot_semantic_attestation_is_rejected_without_native_verifier(self):
        assert_provider_snapshot_rejects_semantic_attestation(self)

    def test_receipt_provider_snapshot_context_binding(self):
        assert_receipt_provider_snapshot_context_binding(self)

    def test_receipt_lifecycle_snapshot_is_fail_closed(self):
        assert_receipt_lifecycle_snapshot_is_fail_closed(self)

    def test_receipt_verification_clock_context_binding(self):
        assert_receipt_verification_clock_context_binding(self)

    def test_live_provider_auth_inputs_are_external(self):
        assert_live_provider_auth_inputs_are_external(self)
