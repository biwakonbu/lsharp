...
fn emit_wasm_wasmgc_internal(
    ...
    let print_string_import = validation::module_uses_print_string(module);
    ...
    validation::validate_module(module, print_string_import)?;
    ...
    if validation::module_uses_typed_funcref(module) {
        let mut recursive_types = Vec::new();
        for gc_type in &module.gc_types {
            recursive_types.push(validation::wasm_gc_gc_subtype(
                gc_type,
        ...
        for import in &module.imports {
            ...
            recursive_types.push(validation::wasm_gc_function_subtype(
                &import.params,
                &[import.result],
                synthetic_import_offset,
            ));
        ...
        if print_string_import {
            let string_type_index = validation::string_array_type_index(module)?;
            ...
            match &gc_type.kind {
                GcTypeKind::Struct(fields) => {
                    let fields = fields
                        .iter()
                        .map(|field| FieldType {
                            element_type: StorageType::Val(validation::wasm_gc_valtype(
                                field.ty,
                                synthetic_import_offset,
                            )),
                ...
                GcTypeKind::Array(element_type) => {
                    types.ty().array(
                        &StorageType::Val(validation::wasm_gc_valtype(
                            *element_type,
                            synthetic_import_offset,
                        )),
                        true,
        ...
        for import in &module.imports {
            ...
            types.ty().function(
                import
                    ...
                    .iter()
                    .copied()
                    .map(|ty| validation::wasm_gc_valtype(ty, synthetic_import_offset)),
                [validation::wasm_gc_valtype(
                    import.result,
                    synthetic_import_offset,
                )],
            );
    ...
    for function in &module.functions {
        let mut locals = function
            ...
            .map(|ty| {
                (
                    1,
                    validation::wasm_gc_valtype(ty, u32::from(print_string_import)),
                )
            })
            .collect::<Vec<_>>();
        let output_locals = if component_output {
            ...
            Some(instructions::ComponentOutputLocals {
                array: base,
                ptr: base + 1,
                len: base + 2,
                index: base + 3,
        ...
        let emit_options = instructions::WasmGcEmitOptions {
            module,
        ...
        instructions::emit_wasm_gc_instructions(&mut wasm_function, &function.body, &emit_options)?;
