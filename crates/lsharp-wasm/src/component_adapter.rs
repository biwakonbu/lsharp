...
fn resolve_world(wit_dir: &Path, world_name: &str) -> Result<(Resolve, WorldId)> {
    ...
    let world = resolve
        ...
        .map_err(|err| ComponentAdapterError::Error {
            msg: format!(
                "world `{world_name}` の解決に失敗しました ({}): {err:#}",
                wit_dir.display()
            ),
        })?;
    Ok((resolve, world))
...
pub fn embed_component_metadata_for_world(
    ...
    let (resolve, world) = resolve_world(wit_dir, world_name)?;
    embed_component_metadata(wasm_bytes, &resolve, world, StringEncoding::UTF8).map_err(|err| {
