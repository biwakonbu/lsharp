...
#[test]
fn selfhost_native_differential_source_stays_within_file_size_budget() {
    let root = selfhost_project_root();
    let source = root.join("crates/lsharp-wasm/tests/e2e/selfhost_native_differential.rs");
    let lines = line_count(&source);
    assert!(
        lines <= 800,
        "selfhost_native_differential.rs は 500〜800 行の責務単位へ分割すること: {lines} 行"
    );
    let manifest = fs::read_to_string(&source)
        .unwrap_or_else(|error| panic!("{} 読み込み失敗: {error}", source.display()));

    let fragment_dir = root.join("crates/lsharp-wasm/tests/e2e/selfhost_native_differential");
    ...
    let expected_includes = fragments
        ...
        .map(|path| {
            format!(
                "include!(\"selfhost_native_differential/{}\");",
                path.file_name()
    ...
    let actual_includes = manifest
        .lines()
        .filter(|line| line.starts_with("include!(\"selfhost_native_differential/"))
        .map(str::to_owned)
        .collect::<Vec<_>>();
    assert_eq!(
