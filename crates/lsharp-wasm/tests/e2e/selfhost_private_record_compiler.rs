...
/// EC-M1-01: private record constructor/accessor を CompilerMode runtime へ接続すること
#[test]
fn test_e2e_selfhost_compiler_mode_private_record_constructor_runs() {
    let harness = r#"
(defn print-bytes-loop [bytes idx count]
  (if (>= idx count)
    0
...
(defn main []
  (let [source "(private (type Secret (record (: x Int)))) (defn main [] (let [value (Secret 41)] (print (Secret.x value)) 0))"
        program (parse-program source)
        pair (compile-program-functions-with-base program 11)
        functions (vector-get pair 1)
        wasm-bytes (build-wasm-bytes-wasi functions (vector-new 0))]
...
"#;
    ...
    .expect("selfhost private record constructor module should run");
    assert_eq!(output, "41\n");
}
