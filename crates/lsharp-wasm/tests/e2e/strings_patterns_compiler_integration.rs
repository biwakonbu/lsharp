...
(defn main []
  (let [body (emit-standalone-write-file-body)]
    (do
      (print (scan-close-set body 0 (vector-length body)))
      0)))
"#;
    ...
    assert_eq!(
        output.trim(),
        "1",
        "write-file の fd_close errno を local へ保存する body が必要"
    );
}

/// selfhost WasmEmit.ls: raw write-file-bytes が fd_close の errno を保持できること
#[test]
fn test_e2e_selfhost_wasmemit_write_file_bytes_preserves_fd_close_errno() {
    let harness = r#"
(defn scan-close-set [body index limit]
  (if (>= index (- limit 4))
    0
...
(defn main []
  (let [body (emit-standalone-write-file-bytes-body)]
    (do
      (print (scan-close-set body 0 (vector-length body)))
      0)))
"#;
    ...
    assert_eq!(
        output.trim(),
        "1",
        "write-file-bytes の fd_close errno を local へ保存する body が必要"
    );
}
...
/// selfhost compiler-mode: map-contains? と map-remove を生成 Wasm 上で実行できること
#[test]
fn test_e2e_selfhost_compiler_mode_map_contains_remove_runs() {
    let harness = r#"
(defn print-bytes-loop [bytes idx count]
  (if (>= idx count)
    0
...
(defn main []
  (let [source "(defn main [] (let [m1 (map-insert (map-new) 123 456)] (let [present (map-contains? m1 123)] (let [missing (map-contains? m1 999)] (let [m2 (map-remove m1 123)] (do (print present) (print missing) (print (map-contains? m2 123)) (print (map-size m2)) 0))))))"
        program (parse-program source)
        pair (compile-program-functions-with-source source program)
        functions (vector-get pair 1)
        data (vector-get pair 2)
...
"#;
    ...
    .expect("selfhost compiler-mode map-contains/remove module should run");
    let lines: Vec<&str> = output.trim().lines().collect();
    assert_eq!(lines, vec!["1", "0", "0", "0"]);
}

/// selfhost compiler-mode の ftable 経路でも map-contains? と map-remove を実行できること
#[test]
fn test_e2e_selfhost_ftable_compiler_mode_map_contains_remove_runs() {
    let harness = r#"
(defn print-bytes-loop [bytes idx count]
  (if (>= idx count)
    0
...
"#;
    ...
    .expect("selfhost ftable compiler-mode map-contains/remove module should run");
    let lines: Vec<&str> = output.trim().lines().collect();
    assert_eq!(lines, vec!["1", "0", "0", "0"]);
}

/// selfhost compiler-mode: source 経路の string key でも map-contains? と map-remove を実行できること
#[test]
fn test_e2e_selfhost_compiler_mode_map_string_key_contains_remove_runs() {
    let harness = r#"
(defn print-bytes-loop [bytes idx count]
  (if (>= idx count)
    0
...
"#;
    ...
    .expect("selfhost compiler-mode string-key map module should run");
    let lines: Vec<&str> = output.trim().lines().collect();
    assert_eq!(lines, vec!["1", "0", "0", "0"]);
}

/// selfhost compiler-mode の ftable 経路でも string key の map lookup を保持すること
#[test]
fn test_e2e_selfhost_ftable_compiler_mode_map_string_key_contains_remove_runs() {
    let harness = r#"
(defn print-bytes-loop [bytes idx count]
  (if (>= idx count)
    0
...
"#;
    ...
    .expect("selfhost ftable compiler-mode string-key map module should run");
    let lines: Vec<&str> = output.trim().lines().collect();
    assert_eq!(lines, vec!["1", "0", "0", "0"]);
}
...
/// selfhost compiler-mode: nested record pattern が親・子の field を bind できること
#[test]
fn test_e2e_selfhost_compiler_mode_nested_record_pattern_runs() {
    let harness = r#"
(defn print-bytes-loop [bytes idx count]
  (if (>= idx count)
    0
...
"#;
    ...
    .expect("selfhost compiler-mode nested record pattern module should run");
    assert_eq!(output, "41\n1\n7\n");
}

/// selfhost compiler-mode: parametric nested record pattern が concrete field を実行できること
#[test]
fn test_e2e_selfhost_compiler_mode_nested_parametric_record_pattern_runs() {
    let harness = r#"
(defn print-bytes-loop [bytes idx count]
  (if (>= idx count)
    0
...
(defn main []
  (let [source "(type (Box a) (record (: value a))) (type (Outer a) (record (: inner (Box a)))) (defn read-inner [o] (match o [{Outer inner {Box value x}} x] [_ 0])) (defn read-literal [o] (match o [{Outer inner {Box value 41}} 1] [_ 0])) (defn read-literal-miss [o] (match o [{Outer inner {Box value 42}} 1] [_ 7])) (defn main [] (let [p {Outer inner {Box value 41}}] (do (print (read-inner p)) (print (read-literal p)) (print (read-literal-miss p)) 0)))"
        program (parse-program source)
        pair (compile-program-functions-with-source source program)
        functions (vector-get pair 1)
        data (vector-get pair 2)
...
"#;
    ...
    .expect("selfhost compiler-mode nested parametric record pattern module should run");
    assert_eq!(output, "41\n1\n7\n");
}
...
(defn main []
  (let [source "(type Point (record (: x Int))) (type Size (record (: x Int))) (defn pick [p] (match p [{Size x x} 1] [_ 0])) (defn main [] (let [p {Point x 41}] (print (pick p)) 0))"
        program (parse-program source)
        pair (compile-program-functions-with-source source program)
        functions (vector-get pair 1)
        data (vector-get pair 2)
...
"#;
    ...
    .expect("selfhost compiler-mode nominal record pattern module should run");
    assert_eq!(output, "0\n");
}
...
/// selfhost compiler-mode: ordinary ADT の nested constructor pattern を actual Wasm で実行できること
#[test]
fn test_e2e_selfhost_compiler_mode_adt_nested_constructor_pattern_runs() {
    let harness = r#"
(defn print-bytes-loop [bytes idx count]
  (if (>= idx count)
    0
...
(defn main []
  (let [source "(type Point (record (: x Int) (: y Int) (: z Int))) (defn main [] (let [p {Point x 1 y 2 z 3} q {p | x 10} r {q | y 20}] (do (print (Point.x q)) (print (Point.y q)) (print (Point.z q)) (print (. q y)) (print (Point.x p)) (print (Point.y p)) (print (Point.z p)) (print (Point.x r)) (print (Point.y r)) (print (Point.z r)) (print (. r x)) (print (. r y)) (print (. r z)) (print (match p [{Point x x} x] [_ 0])) (print (match p [{Point x 42} 1] [_ 0])) (print (match q [{Point x x} x] [_ 0])) (print (match r [{Point x x} x] [_ 0])) 0)))"
        program (parse-program source)
        pair (compile-program-functions-with-source source program)
        functions (vector-get pair 1)
        data (vector-get pair 2)
...
/// selfhost ftable compiler: alias-qualified function call を actual Wasm で実行できること
#[test]
fn test_e2e_selfhost_ftable_compiler_alias_qualified_function_call_runs() {
    let harness = r#"
(defn print-bytes-loop [bytes idx count]
  (if (>= idx count)
    0
...
(defn main []
  (let [source "(defn inc [n] (+ n 1)) (import App.Math :as M :only [inc]) (defn main [] (do (print (M.inc 41)) 0))"
        program (parse-program source)
        pair (compile-program-functions-with-base program 11)
        functions (vector-get pair 1)
        wasm-bytes (build-wasm-bytes-wasi functions (vector-new 0))]
...
"#;
    ...
    .expect("selfhost ftable alias-qualified function module should run");
    assert_eq!(output, "42\n");
}
...
/// selfhost ftable compiler: alias-qualified record literal を pattern と nominal に接続できること
#[test]
fn test_e2e_selfhost_ftable_compiler_alias_qualified_record_literal_pattern_runs() {
    let harness = r#"
(defn print-bytes-loop [bytes idx count]
  (if (>= idx count)
    0
...
(defn main []
  (let [source "(type Point (record (: x Int) (: y Int))) (import App.Shapes :as S :only [Point]) (defn main [] (let [point {S.Point x 41 y 2}] (print (match point [{S.Point x x} x] [_ 0])) 0))"
        program (parse-program source)
        pair (compile-program-functions-with-base program 11)
        functions (vector-get pair 1)
        wasm-bytes (build-wasm-bytes-wasi functions (vector-new 0))]
...
"#;
    ...
    .expect("selfhost ftable alias-qualified record literal pattern module should run");
    assert_eq!(output, "41\n");
}
...
/// selfhost ftable compiler: deep nested parametric record pattern を再帰的に実行できること
#[test]
fn test_e2e_selfhost_ftable_compiler_deep_nested_parametric_record_pattern_runs() {
    let harness = r#"
(defn print-bytes-loop [bytes idx count]
  (if (>= idx count)
    0
...
(defn main []
  (let [source "(type (Box a) (record (: value a))) (type (Middle a) (record (: box (Box a)))) (type (Outer a) (record (: middle (Middle a)))) (type (Root a) (record (: outer (Outer a)))) (import App.Shapes :as S :only [Box Middle Outer Root]) (defn main [] (let [r {S.Root outer {S.Outer middle {S.Middle box {S.Box value 41}}}}] (do (print (match r [{S.Root outer {S.Outer middle {S.Middle box {S.Box value x}}}} x] [_ 0])) (print (match r [{S.Root outer {S.Outer middle {S.Middle box {S.Box value 41}}}} 1] [_ 0])) (print (match r [{S.Root outer {S.Outer middle {S.Middle box {S.Box value 42}}}} 1] [_ 7])) 0)))"
        program (parse-program source)
        pair (compile-program-functions-with-base program 11)
        functions (vector-get pair 1)
        data (vector-get pair 2)
...
"#;
    ...
    .expect("selfhost ftable deep nested parametric record pattern module should run");
    assert_eq!(output, "41\n1\n7\n");
}
...
(defn main []
  (let [source "(type (Box a) (record (: value a))) (import App.Shapes :as S :only [Box]) (defn main [] (let [int-box {S.Box value 41} bool-box {S.Box value true}] (do (print (match int-box [{S.Box value x} x] [_ 0])) (print (if (match bool-box [{S.Box value x} x] [_ false]) 1 0)) 0)))"
        program (parse-program source)
        pair (compile-program-functions-with-base program 11)
        functions (vector-get pair 1)
        wasm-bytes (build-wasm-bytes-wasi functions (vector-new 0))]
...
"#;
    ...
    .expect("selfhost ftable alias-qualified parametric multi-instantiation module should run");
    assert_eq!(output, "41\n1\n");
}
...
(defn main []
  (let [source "(type Point (record (: x Int) (: y Int))) (defn main [] (let [p {Point x 1 y 2} q {p | x 10} r {q | y 20}] (do (print (match q [{Point x x} x] [_ 0])) (print (match q [{Point x 42} 1] [_ 0])) (print (match r [{Point x x} x] [_ 0])) 0)))"
        program (parse-program source)
        pair (compile-program-functions-with-base program 11)
        functions (vector-get pair 1)
        wasm-bytes (build-wasm-bytes-wasi functions (vector-new 0))]
...
"#;
    ...
    .expect("selfhost ftable compiler record pattern module should run");
    assert_eq!(output, "10\n0\n10\n");
}

/// selfhost ftable compiler: nested record の binder と literal fallback を実行できること
#[test]
fn test_e2e_selfhost_ftable_compiler_nested_record_pattern_runs() {
    let harness = r#"
(defn print-bytes-loop [bytes idx count]
  (if (>= idx count)
    0
...
/// selfhost compiler-mode: alias-qualified import 先 record を update して actual Wasm で実行できること
#[test]
fn test_e2e_selfhost_compiler_mode_imported_record_update_runs() {
    let temp_root = std::env::temp_dir().join(format!(
        "lsharp-selfhost-record-import-update-runtime-{}",
        std::process::id()
    ));
    let _ = std::fs::remove_dir_all(&temp_root);
    let app_dir = temp_root.join("src/App");
    ...
    .expect("record update import fixture の Shapes.ls を書けない");
    std::fs::write(
        app_dir.join("Main.ls"),
        "(module App.Main)\n(import App.Shapes :as S :only [Point])\n(defn main [] (let [point (S.Point 40 2) updated {point | x 41}] (do (print (. updated x)) (print (. updated y)) 0)))\n",
    )
    .expect("record update import fixture の Main.ls を書けない");
...
fn test_e2e_selfhost_compiler_mode_imported_alias_qualified_parametric_record_literal_pattern_runs() {
    ...
    std::fs::create_dir_all(&app_dir)
        .expect("parametric record literal import fixture の directory を作れない");
    std::fs::write(
        app_dir.join("Shapes.ls"),
        "(module App.Shapes)\n(type (Box a) (record (: value a)))\n",
    )
    .expect("parametric record literal import fixture の Shapes.ls を書けない");
    std::fs::write(
        app_dir.join("Main.ls"),
        "(module App.Main)\n(import App.Shapes :as S :only [Box])\n(defn main [] (let [box {S.Box value 41}] (print (match box [{S.Box value x} x] [_ 0])) 0))\n",
    )
    .expect("parametric record literal import fixture の Main.ls を書けない");
...
fn test_e2e_selfhost_compiler_mode_imported_alias_qualified_same_schema_record_patterns_are_nominal() {
    ...
    let app_dir = temp_root.join("src/App");
    std::fs::create_dir_all(&app_dir)
        .expect("same-schema record pattern fixture の directory を作れない");
    std::fs::write(
        app_dir.join("Left.ls"),
        "(module App.Left)\n(type Point (record (: x Int)))\n",
    )
    .expect("same-schema record pattern fixture の Left.ls を書けない");
    std::fs::write(
        app_dir.join("Right.ls"),
        "(module App.Right)\n(type Point (record (: x Int)))\n",
    )
    .expect("same-schema record pattern fixture の Right.ls を書けない");
    std::fs::write(
        app_dir.join("Main.ls"),
        "(module App.Main)\n(import App.Left :as L :only [Point])\n(import App.Right :as R :only [Point])\n(defn main [] (let [left (L.Point 41)] (do (print (match left [{R.Point x x} 1] [_ 0])) 0)))\n",
    )
    .expect("same-schema record pattern fixture の Main.ls を書けない");
    ...
    .expect("same-schema alias-qualified record patterns を含む selfhost compiler-mode module should run");
    assert_eq!(output, "0\n");
