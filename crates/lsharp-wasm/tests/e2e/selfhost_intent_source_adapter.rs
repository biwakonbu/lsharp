...
fn run_source_evidence_runtime(harness: &str) -> String {
    ...
    let whitespace_source = std::fs::read_to_string(
        selfhost_project_root().join("selfhost/src/Tools/Validation/Whitespace.ls"),
    )
    .expect("canonical Whitespace.ls が読み込めない");
...
/// EC-M2-02: empty coverage bucket は deterministic sampling の empty-field code 4 で拒否する。
#[test]
fn test_e2e_selfhost_source_evidence_rejects_empty_coverage_bucket() {
    let harness = r#"
(defn main []
  (let [result (source-evidence-graph-from-program
                 (parse-program "(defn cancel [] :claim \"claim:checkout/cancel\" \"The API rejects shipped orders\" :evidence \"evidence:checkout/empty-coverage\" :subject \"claim:checkout/cancel\" :method \"property\" :outcome \"pass\" :runner \"source-empty-coverage\" :target \"aarch64-apple-darwin\" :source-commit \"source-empty-coverage-commit\" :artifact-digest \"sha256:source-empty-coverage\" :cases 1 :seed 0 :generator \"source-empty-coverage-generator\" :coverage [(\"\" 1)] :producer \"source-empty-coverage-producer\" :tool-version \"0.2.0-dev\" :timestamp \"2026-07-29T00:00:00Z\" :independence \"same-author\" true)"))
        error (source-result-error result)]
    (do
      (print (source-result-status result))
      (print (source-evidence-error-code error))
...
"#;
    ...
    assert_eq!(
        lines,
        ["0", "4", "coverage", "", "80", "124"],
        "empty coverage bucket は source evidence の empty-field code 4 と directive span で拒否するべき"
    );
}
...
/// EC-M2-02: evidence subject の不正 ID は code 2 と directive span を返す。
#[test]
fn test_e2e_selfhost_source_evidence_preserves_invalid_subject_id_span() {
    let harness = r#"
(defn main []
  (let [nodes (vector-push-single-rooted-v3
                (vector-new 0)
                (source-node-record (source-node-claim) "claim:checkout/cancel" "The API rejects shipped orders" 1 2))
        payload (source-evidence-payload
                  "evidence:checkout/subject-id"
                  "  "
                  "case"
                  "pass"
                  "evidence-subject-runner"
                  "aarch64-apple-darwin"
                  "source-evidence-subject-id"
                  "sha256:evidence-subject-id"
                  1
                  0
                  "evidence-subject-generator"
                  (vector-new 0)
                  (vector-new 0)
                  "evidence-subject-producer"
                  "0.2.0-dev"
                  "2026-07-28T00:00:00Z"
                  "same-author")
        form (source-evidence-form payload 10 20)
...
"#;
    ...
    assert_eq!(
        lines,
        ["0", "2", "subject", "  ", "10", "20"],
        "invalid evidence subject ID は code 2 と directive span を保持するべき"
    );
}
...
/// EC-M3-04: optional `expires-at` を省略した source attestation も Rust canonical bytes と
/// selfhost producer で同じ空 field を保持し、unverified/span の境界を失わない。
#[test]
fn test_e2e_selfhost_source_adapter_canonical_bytes_match_rust_without_expiry() {
    let harness = r#"
(defn emit-bytes [bytes idx]
  (if (>= idx (vector-length bytes))
    0
...
(defn main []
  (let [result (source-review-attestations-from-program
                 (parse-program "(defn review [] :review-attestation :review-id \"review:checkout/reviewer-001\" :subject-digest \"sha256:subject-001\" :source-commit \"0123456789abcdef\" :provenance-digest \"sha256:review-001\" :provider \"github\" :key-id \"org/reviews-2026\" :algorithm \"ed25519\" :signature \"AAECAw\" :issued-at \"2026-08-01T00:00:00Z\" :sequence 3 true)"))
        records (source-result-value result)
        attestation (vector-get records 0)
        bytes (source-review-attestation-canonical-bytes attestation)]
    (do
...
"#;
    ...
    .expect("Rust optional expires-at fixture は valid であるべき")
    .canonical_bytes();
