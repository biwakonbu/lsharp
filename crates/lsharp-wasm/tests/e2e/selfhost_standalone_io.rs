...
fn test_e2e_selfhost_standalone_read_stdin_runtime() {
    run_with_expanded_stack(NATIVE_HARNESS_STACK_BYTES, || {
        ...
        let boundary_over_4m = format!("{}b", "a".repeat((4 * 1024 * 1024) - 1));
        let cases = vec![
...
#[test]
fn test_e2e_selfhost_standalone_read_file_returns_all_bytes_over_4m() {
    run_with_expanded_stack(NATIVE_HARNESS_STACK_BYTES, || {
        let dir = fixture_dir("read_over_4m");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).expect("4MiB read fixture directory の作成に失敗");
        let mut payload = vec![b'a'; (4 * 1024 * 1024) - 1];
        payload.push(b'b');
        std::fs::write(dir.join("input.txt"), &payload)
            .expect("4MiB read fixture input.txt の書き込みに失敗");

        let standalone_wasm = compile_standalone_source(
            r#"(defn main [] (print-string (read-file "input.txt")))"#,
            &dir,
            "LSHARP_STANDALONE_IO_READ_OVER_4M_CLI_ARTIFACT",
        );
        let output = lsharp_wasm::wasi_runner::run_wasm_wasi_with_dir_args_and_stdin_capture(
            &standalone_wasm,
            Some(&dir),
        ...
        .expect("4MiB read の standalone 実行に失敗");
        let _ = std::fs::remove_dir_all(&dir);
