...
fn parser_defn_body_finalize_uses_small_rooted_helper() {
    ...
    assert!(
        body.contains("(finalize-defn-body-v3 parsed-defn-body defn-node)")
...
fn parser_parse_defn_inlines_non_meta_body_after_rooted_param_parse() {
    ...
    assert!(
        parse_defn.contains("defn-node (vector-set-at-rooted-v3 with-params 2 param-count)")
            ...
            && parse_defn.contains("(let [parsed-defn-body (parse-expr-v3 spans pos-ref src)]")
            && parse_defn.contains("(finalize-defn-body-v3 parsed-defn-body defn-node)")
...
#[test]
fn compiler_base_step_state_keeps_root_safe_builder_and_progress_probe() {
    let source =
    ...
    assert!(
        body.contains("compile-step-state-progress-mode")
            ...
            && body.contains("base0 (push-int-vector-local (vector-new 3) done)")
            && body.contains("base1 (push-int-vector-local base0 next-idx)")
            && body.contains("(root_set value-slot state)")
            && body.contains("state-root (root_push state)"),
        "make-compile-step-state は root-safe local builder を維持しつつ entry/result handoff probe を出せるべき"
    );
}
