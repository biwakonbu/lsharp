...
fn test_e2e_selfhost_cli_lsp_stdio_body_hover_spec_position_character_params() {
    let hover_body = r#"{"jsonrpc":"2.0","id":66,"method":"textDocument/hover","params":{"textDocument":{"uri":42},"position":{"line":0,"character":38}}}"#;
...
"#
    ...
    assert_eq!(
        lines,
        vec!["42", "1", "39"],
        // wire は 0 始まり、内部は 1 始まり。lsp-stdio-nav-params が line/col の双方へ +1 する
        // ので、wire (0, 38) は内部 (1, 39) として読まれるのが正しい (AGENTS.md の座標規約)
        "spec hover params は [uri,line,col]=[42,1,39] として読まれるべき: {:?}",
        lines
    );
}
...
fn test_e2e_selfhost_cli_lsp_stdio_body_rename_spec_position_character_params() {
    let rename_body = r#"{"jsonrpc":"2.0","id":70,"method":"textDocument/rename","params":{"textDocument":{"uri":42},"position":{"line":0,"character":38},"newName":"cube"}}"#;
...
"#
    ...
    assert_eq!(
        lines,
        vec!["42", "1", "39", "cube"],
        // 同上。col だけ wire 値のままを期待していたのが陳腐化していた
        "spec rename params は [uri,line,col,newName]=[42,1,39,cube] として読まれるべき: {:?}",
        lines
    );
}
...
/// EC-M1-02/05: selfhost runner が property の String literal を Rust oracle と評価すること
#[test]
fn test_e2e_selfhost_runner_matches_rust_oracle_for_string_property_literal() {
    ...
    let harness = r#"
(defn main []
  (let [src "(defn constant [sample] :property [(for-all [sample String] :cases 1 :postcondition (string-eq result \"ok\"))] \"ok\")"
        suite (generate-tests-from-source src)
        properties (vector-get suite 4)
        result0 (vector-get properties 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines,
        vec!["1", "1", "1", "0"],
        "selfhost String literal property は Rust oracle と同じ success を返すべき"
    );
}

/// EC-M1-02/05: selfhost runner が property precondition の String literal を Rust oracle と評価すること
#[test]
fn test_e2e_selfhost_runner_matches_rust_oracle_for_string_property_precondition_literal() {
    ...
    let harness = r#"
(defn main []
  (let [src "(defn constant [input] :property [(for-all [sample String] :cases 2 :precondition [(string-eq sample \"a\")] :postcondition (string-eq result \"ok\"))] \"ok\")"
        suite (generate-tests-from-source src)
        properties (vector-get suite 4)
        result0 (vector-get properties 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines,
        vec!["1", "1", "1", "0"],
        "selfhost String precondition property は Rust oracle と同じ success を返すべき"
    );
}
...
fn test_e2e_selfhost_cli_test_source_json_reports_literal_non_bool_invariant_message() {
    ...
    let harness = r#"
(defn main []
  (let [src "(defn succ [x] :invariant 1 (+ x 1))"]
    (print (run-test-source src 1))))
"#;
...
fn test_e2e_selfhost_cli_test_source_json_reports_string_non_bool_invariant_message() {
    ...
    let harness = r#"
(defn main []
  (let [src "(defn text [x] :invariant \"bad\" (+ x 1))"]
    (print (run-test-source src 1))))
"#;
...
fn test_e2e_selfhost_test_runner_matches_rust_oracle_for_valid_invariant_match_string_literal() {
    ...
    let harness = r#"
(defn main []
  (let [src "(defn label [] :invariant (match true [true (string-eq result \"ok\")]) \"ok\")"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines,
        vec!["1", "1", "1", "0"],
        "selfhost match String evaluator は Rust oracle と同じ success を返すべき"
    );
}
...
fn test_e2e_selfhost_test_runner_rejects_non_bool_unselected_match_arm() {
    ...
    let harness = r#"
(defn main []
  (let [src "(type (Maybe a) (Just a) Nothing) (defn make-just [x] (Just x)) (defn check [x] :invariant (match (make-just x) [(Just value) true] [Nothing (+ x 1)]) x)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(1).copied(),
        Some("2"),
        "selfhost は未選択 arm の non-Bool を LS1002 として拒否するべき"
    );
    assert_eq!(
        lines.get(2).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost unselected match arm の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost unselected match arm の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_rejects_non_bool_match_guard() {
    ...
    let harness = r#"
(defn main []
  (let [src "(defn check [] :invariant (match true [_ when (+ 1 2) true] [_ true]) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(2).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost match guard の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost match guard の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost match guard message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost match guard message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_if_diagnostic_message() {
    ...
    let harness = r#"
(defn main []
  (let [src "(defn check [] :invariant (if true (+ 1 2) false) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost if message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost if message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_diagnostic_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (+ 1 delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_user_function_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は user-defined return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn always-int [] (+ 1 2)) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (always-int))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation user function message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation user function message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_one_arg_function_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は one-arg user-defined return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn increment [x] (+ x 1)) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (increment delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation one-arg function message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation one-arg function message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_one_arg_identity_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は one-arg identity return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (identity 1))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation one-arg identity message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation one-arg identity message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_two_arg_function_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は two-arg user-defined return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn add [x y] (+ x y)) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (add 1 2))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation two-arg function message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation two-arg function message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_bound_identity_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は bound identity return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (identity delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation bound identity message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation bound identity message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_bound_if_function_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は bound if function return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn choose-int [x] (if true (+ x 1) 0)) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (choose-int delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation bound if function message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation bound if function message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_bound_let_function_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は bound let function return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn choose-int [x] (let [next (+ x 1)] next)) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (choose-int delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation bound let function message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation bound let function message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_bound_match_function_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は bound match function return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn choose-int [x] (match true [_ (+ x 1)] [_ 0])) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (choose-int delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation bound match function message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation bound match function message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_bound_do_function_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は bound do function return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn choose-int [x] (do 0 (+ x 1))) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (choose-int delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation bound do function message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation bound do function message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_bound_lambda_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は bound lambda return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return ((fn [x] (+ x 1)) delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation bound lambda message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation bound lambda message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_bound_lambda_control_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は bound lambda control return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return ((fn [x] (if true x x)) delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation bound lambda control message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation bound lambda control message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_bound_user_function_control_message()
 {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は bound user function control return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn choose-int [x] (if true x x)) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (choose-int delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation bound user function control message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation bound user function control message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_bound_higher_order_function_message()
 {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は bound higher-order function return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn increment [x] (+ x 1)) (defn apply-one [f x] (f x)) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (apply-one increment delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation bound higher-order function message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation bound higher-order function message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_bound_inline_lambda_higher_order_message()
 {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は bound inline lambda higher-order return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn apply-one [f x] (f x)) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (apply-one (fn [x] (+ x 1)) delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation bound inline lambda higher-order message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation bound inline lambda higher-order message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_bound_closure_capture_higher_order_message()
 {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は bound closure capture higher-order return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn make-constant [delta] (fn [x] (if true delta delta))) (defn apply-one [f x] (f x)) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (apply-one (make-constant delta) delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation bound closure capture higher-order message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation bound closure capture higher-order message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_bound_nested_closure_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は bound nested closure return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation bound nested closure message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation bound nested closure message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_three_arg_lambda_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は three-arg lambda return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return ((fn [x y z] (+ x (+ y z))) delta delta delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation three-arg lambda message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation three-arg lambda message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_three_arg_user_function_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は three-arg user function return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn sum3 [x y z] (+ x (+ y z))) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (sum3 delta delta delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation three-arg user function message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation three-arg user function message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_four_arg_user_function_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は four-arg user function return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn sum4 [x y z q] (+ x (+ y (+ z q)))) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (sum4 delta delta delta delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation four-arg user function message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation four-arg user function message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_six_arg_user_function_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は six-arg user function return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn sum6 [x y z q r s] (+ x (+ y (+ z (+ q (+ r s)))))) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (sum6 delta delta delta delta delta delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation six-arg user function message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation six-arg user function message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_seven_arg_user_function_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は seven-arg user function return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn sum7 [x y z q r s t] (+ x (+ y (+ z (+ q (+ r (+ s t))))))) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (sum7 delta delta delta delta delta delta delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation seven-arg user function message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation seven-arg user function message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_eight_arg_user_function_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は eight-arg user function return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn sum8 [x y z q r s t u] (+ x (+ y (+ z (+ q (+ r (+ s (+ t u)))))))) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (sum8 delta delta delta delta delta delta delta delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation eight-arg user function message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation eight-arg user function message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_nine_arg_user_function_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は nine-arg user function return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn sum9 [x y z q r s t u v] (+ x (+ y (+ z (+ q (+ r (+ s (+ t (+ u v))))))))) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (sum9 delta delta delta delta delta delta delta delta delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation nine-arg user function message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation nine-arg user function message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_ten_arg_user_function_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は ten-arg user function return の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn sum10 [x y z q r s t u v w] (+ (+ (+ (+ x y) (+ z q)) (+ (+ r s) (+ t u))) (+ v w))) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (sum10 delta delta delta delta delta delta delta delta delta delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation ten-arg user function message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation ten-arg user function message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_three_arg_lambda_returning_closure_message()
 {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は three-arg lambda returning closure の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return (((fn [x y z] (fn [w] (+ x w))) delta delta delta) delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation three-arg lambda returning closure message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation three-arg lambda returning closure message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_three_arg_user_function_returning_closure_message()
 {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は three-arg user function returning closure の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn make-constant3 [x y z] (fn [w] (+ x w))) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return ((make-constant3 delta delta delta) delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation three-arg user function returning closure message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation three-arg user function returning closure message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_four_arg_user_function_returning_closure_message()
 {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は four-arg user function returning closure の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn make-constant4 [x y z q] (fn [w] (+ x (+ y (+ z (+ q w)))))) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return ((make-constant4 delta delta delta delta) delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation four-arg user function returning closure message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation four-arg user function returning closure message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_five_arg_user_function_returning_closure_message()
 {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は five-arg user function returning closure の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn make-constant5 [x y z q r] (fn [w] (+ x (+ y (+ z (+ q (+ r w))))))) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return ((make-constant5 delta delta delta delta delta) delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation five-arg user function returning closure message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation five-arg user function returning closure message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_six_arg_user_function_returning_closure_message()
 {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は six-arg user function returning closure の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn make-constant6 [x y z q r s] (fn [w] (+ x (+ y (+ z (+ q (+ r (+ s w)))))))) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return ((make-constant6 delta delta delta delta delta delta) delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation six-arg user function returning closure message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation six-arg user function returning closure message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_seven_arg_user_function_returning_closure_message()
 {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は seven-arg user function returning closure の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn make-constant7 [x y z q r s t] (fn [w] (+ x (+ y (+ z (+ q (+ r (+ s (+ t w))))))))) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return ((make-constant7 delta delta delta delta delta delta delta) delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation seven-arg user function returning closure message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation seven-arg user function returning closure message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_eight_arg_user_function_returning_closure_message()
 {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は eight-arg user function returning closure の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn make-constant8 [x y z q r s t u] (fn [w] (+ x (+ y (+ z (+ q (+ r (+ s (+ t (+ u w)))))))))) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return ((make-constant8 delta delta delta delta delta delta delta delta) delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation eight-arg user function returning closure message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation eight-arg user function returning closure message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_nine_arg_user_function_returning_closure_message()
 {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は nine-arg user function returning closure の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn make-constant9 [x y z q r s t u v] (fn [w] (+ x (+ y (+ z (+ q (+ r (+ s (+ t (+ u (+ v w))))))))))) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return ((make-constant9 delta delta delta delta delta delta delta delta delta) delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation nine-arg user function returning closure message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation nine-arg user function returning closure message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_computation_ten_arg_user_function_returning_closure_message()
 {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は ten-arg user function returning closure の non-Bool computation を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(computation-builder maybe-builder mb identity) (defn identity [x] x) (defn mb [m f] (f m)) (defn make-constant10 [x y z q r s t u v w] (fn [k] (+ (+ (+ (+ x y) (+ z q)) (+ (+ r s) (+ t u))) (+ (+ v w) k)))) (defn check [] :invariant (computation maybe-builder (let! delta 1) (return ((make-constant10 delta delta delta delta delta delta delta delta delta delta) delta))) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost computation ten-arg user function returning closure message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost computation ten-arg user function returning closure message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_rejects_non_bool_function_match_guard() {
    ...
    let harness = r#"
(defn main []
  (let [src "(defn always-int [] (+ 1 2)) (defn check [] :invariant (match true [_ when (always-int) true] [_ true]) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
fn test_e2e_selfhost_test_runner_preserves_non_bool_three_arg_function_match_guard_message() {
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "Rust oracle は three-argument function の non-Bool guard を 1 件診断するべき: {diagnostics:?}"
    );
    let oracle_message = diagnostics[0].message.clone();
    let oracle_span = diagnostics[0].span;
    ...
    let harness = r#"
(defn main []
  (let [src "(defn sum3 [x y z] (+ x (+ y z))) (defn check [] :invariant (match true [_ when (sum3 1 2 3) true] [_ true]) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(2).copied(),
        Some(oracle_message.as_str()),
        "selfhost three-argument function guard message は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost three-argument function guard message の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(4).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost three-argument function guard message の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
"#;
    ...
    assert_eq!(
        lines.get(1).copied(),
        Some("2"),
        "selfhost は unknown return shape の non-Bool guard を LS1002 として拒否するべき"
    );
    assert_eq!(
        lines.get(2).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost dynamic function guard の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost dynamic function guard の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_rejects_non_bool_lambda_match_guard() {
    ...
    assert_eq!(
        &source[oracle_span.start..oracle_span.end],
        "((fn [x] x) 1)",
        "Rust oracle は lambda call 全体の span を返すべき"
    );

    let harness = r#"
(defn main []
  (let [src "(defn check [] :invariant (match true [_ when ((fn [x] x) 1) true] [_ true]) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(1).copied(),
        Some("2"),
        "selfhost は lambda call の non-Bool guard を LS1002 として拒否するべき"
    );
    assert_eq!(
        lines.get(2).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost lambda guard の diagnostic span 開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(3).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost lambda guard の diagnostic span 終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_test_runner_matches_rust_oracle_for_valid_invariant_match_guard() {
    let source = r#"
(defn label []
  :invariant (match 1 [when when false true] [_ true])
  true)
(defn label-true []
  :invariant (match 1 [value when true true] [_ false])
  true)
"#;
    let oracle = run_metadata_tests(source);
    assert_eq!(
    ...
    let harness = r#"
(defn main []
  (let [src "(defn label [] :invariant (match 1 [when when false true] [_ true]) true) (defn label-true [] :invariant (match 1 [value when true true] [_ false]) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result0 (vector-get results 0)
        result1 (vector-get results 1)]
...
"#;
    ...
    assert_eq!(
        lines,
        vec!["2", "1", "1", "0", "1", "1", "0"],
        "selfhost guarded match evaluator は Rust oracle と同じ guard/fall-through success を返すべき"
    );
}
...
(defn point []
  :invariant (match {Point x 41 y 2} [{Point x value} (= value 41)] [_ false])
  true)
"#;
    let oracle = run_metadata_tests(source);
    assert_eq!(
    ...
    let harness = r#"
(defn main []
  (let [src "(type Point (record (: x Int) (: y Int))) (defn point [] :invariant (match {Point x 41 y 2} [{Point x value} (= value 41)] [_ false]) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result (vector-get results 0)]
    (do
...
(type Point (record (: x Int) (: y Int)))
(defn point []
  :invariant (= (. {{Point x 41 y 2} | x 42} x) 42)
  true)
"#;
    let oracle = run_metadata_tests(source);
    assert_eq!(
    ...
    let harness = r#"
(defn main []
  (let [src "(type Point (record (: x Int) (: y Int))) (defn point [] :invariant (= (. {{Point x 41 y 2} | x 42} x) 42) true)"
        suite (generate-tests src)
        results (vector-get suite 1)
        result (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines,
        vec!["1", "1", "1", "0"],
        "selfhost record update evaluator は Rust oracle と同じ updated field success を返すべき"
    );
}
...
"#;
    ...
    assert_eq!(
        ...
        Some(oracle_span.start),
        "selfhost unknown-variable span の開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(2).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost unknown-variable span の終了位置は Rust oracle と一致するべき"
    );
}
...
"#;
    ...
    let lines: Vec<&str> = output.trim().lines().collect();
    assert_eq!(lines.first().copied(), Some("2"));
    assert_eq!(
        lines.get(1).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost non-Bool invariant span の開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(2).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost non-Bool invariant span の終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_property_runner_preserves_non_bool_postcondition_span() {
    ...
    let harness = r#"
(defn main []
  (let [src "(defn identity [x] :property [(for-all [x Int] :cases 1 :postcondition (+ result 1))] x)"
        suite (generate-tests src)
...
"#;
    ...
    assert_eq!(
        lines.get(1).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost non-Bool property span の開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(2).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost non-Bool property span の終了位置は Rust oracle と一致するべき"
    );
}
...
/// EC-M1-02: selfhost property 実行診断が2番目の non-Bool precondition spanを保持すること
#[test]
fn test_e2e_selfhost_property_runner_preserves_second_non_bool_precondition_span() {
    ...
    let harness = r#"
(defn main []
  (let [src "(defn identity [x] :property [(for-all [x Int] :cases 1 :precondition [(>= x 0) (+ x 1)] :postcondition (= result x))] x)"
        suite (generate-tests src)
        results (vector-get suite 4)
        result0 (vector-get results 0)]
    (do
...
"#;
    ...
    assert_eq!(
        lines.get(1).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.start),
        "selfhost second non-Bool precondition span の開始位置は Rust oracle と一致するべき"
    );
    assert_eq!(
        lines.get(2).and_then(|line| line.parse::<usize>().ok()),
        Some(oracle_span.end),
        "selfhost second non-Bool precondition span の終了位置は Rust oracle と一致するべき"
    );
}
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_goto_definition() {
    let request_body = r#"{"jsonrpc":"2.0","id":61,"method":"textDocument/definition","params":{"uri":10,"line":0,"col":38,"source":"(defn helper [x] x) (defn main [] (helper 1))"}}"#;
    let stdin = format!(
        "Content-Length: {}\r\n\r\n{}",
        request_body.len(),
        request_body
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_hover() {
    let request_body = r#"{"jsonrpc":"2.0","id":62,"method":"textDocument/hover","params":{"uri":10,"line":0,"col":38,"source":"(defn helper [x] x) (defn main [] (helper 1))"}}"#;
    let stdin = format!(
        "Content-Length: {}\r\n\r\n{}",
        request_body.len(),
        request_body
    );
    let response_body = r#"{"jsonrpc":"2.0","id":62,"result":{"range":{"start":{"line":0,"character":35},"end":{"line":0,"character":41}},"contents":"defn helper"}}"#;
    let expected = format!(
        "Content-Length: {}\r\n\r\n{}",
        response_body.len(),
        response_body
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_references() {
    let request_body = r#"{"jsonrpc":"2.0","id":63,"method":"textDocument/references","params":{"uri":10,"line":0,"col":38,"source":"(defn square [x] x) (defn main [] (square 1) (square 2))"}}"#;
    let stdin = format!(
        "Content-Length: {}\r\n\r\n{}",
        request_body.len(),
        request_body
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_rename() {
    let request_body = r#"{"jsonrpc":"2.0","id":65,"method":"textDocument/rename","params":{"uri":10,"line":0,"col":38,"source":"(defn square [x] x) (defn main [] (square 1) (square 2))","newName":"cube"}}"#;
    let stdin = format!(
        "Content-Length: {}\r\n\r\n{}",
        request_body.len(),
        request_body
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_hover_uses_open_document() {
    ...
    let hover_body = r#"{"jsonrpc":"2.0","id":66,"method":"textDocument/hover","params":{"uri":42,"line":0,"col":38}}"#;
    let stdin = format!(
        "Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}",
        open_body.len(),
        open_body,
    ...
    let hover_response = r#"{"jsonrpc":"2.0","id":66,"result":{"range":{"start":{"line":0,"character":35},"end":{"line":0,"character":41}},"contents":"defn helper"}}"#;
    let expected = format!(
        "Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}",
        open_response.len(),
        open_response,
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_definition_uses_open_document() {
    ...
    let definition_body = r#"{"jsonrpc":"2.0","id":67,"method":"textDocument/definition","params":{"uri":42,"line":0,"col":38}}"#;
    let stdin = format!(
        "Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}",
        open_body.len(),
        open_body,
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_definition_uses_open_document_spec_params() {
    ...
    let definition_body = r#"{"jsonrpc":"2.0","id":67,"method":"textDocument/definition","params":{"textDocument":{"uri":42},"position":{"line":0,"character":38}}}"#;
    let stdin = format!(
        "Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}",
        open_body.len(),
        open_body,
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_references_uses_open_document() {
    ...
    let references_body = r#"{"jsonrpc":"2.0","id":68,"method":"textDocument/references","params":{"uri":42,"line":0,"col":38}}"#;
    let stdin = format!(
        "Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}",
        open_body.len(),
        open_body,
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_references_uses_open_document_spec_params() {
    ...
    let references_body = r#"{"jsonrpc":"2.0","id":68,"method":"textDocument/references","params":{"textDocument":{"uri":42},"position":{"line":0,"character":38}}}"#;
    let stdin = format!(
        "Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}",
        open_body.len(),
        open_body,
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_rename_uses_open_document() {
    ...
    let rename_body = r#"{"jsonrpc":"2.0","id":70,"method":"textDocument/rename","params":{"uri":42,"line":0,"col":38,"newName":"cube"}}"#;
    let stdin = format!(
        "Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}",
        open_body.len(),
        open_body,
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_definition_resolves_open_document() {
    ...
    let definition_body = r#"{"jsonrpc":"2.0","id":72,"method":"textDocument/definition","params":{"uri":10,"line":0,"col":2}}"#;
    let stdin = format!(
        "Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}",
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_hover_resolves_open_document() {
    ...
    let hover_body = r#"{"jsonrpc":"2.0","id":73,"method":"textDocument/hover","params":{"uri":10,"line":0,"col":2}}"#;
    let stdin = format!(
        "Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}",
        open_helper_body.len(),
        open_helper_body,
    ...
    // didOpen は uri:10 の parse diagnostics frame を後続させる (実測 2026-08-23)
    let main_diagnostics = r#"{"jsonrpc":"2.0","method":"textDocument/publishDiagnostics","params":{"uri":10,"diagnostics":[{"range":{"start":{"line":0,"character":1},"end":{"line":0,"character":7}},"severity":1,"code":"LS0103","source":"lsharp","message":"unknown form"}]}}"#;
    let hover_response = r#"{"jsonrpc":"2.0","id":73,"result":{"range":{"start":{"line":0,"character":1},"end":{"line":0,"character":7}},"contents":"defn helper"}}"#;
    let expected = format!(
        "Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}",
        open_helper_response.len(),
        open_helper_response,
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_hover_uses_changed_document() {
    ...
    let hover_body = r#"{"jsonrpc":"2.0","id":76,"method":"textDocument/hover","params":{"uri":42,"line":0,"col":38}}"#;
    let stdin = format!(
        "Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}",
        open_body.len(),
        open_body,
    ...
    let hover_response = r#"{"jsonrpc":"2.0","id":76,"result":{"range":{"start":{"line":0,"character":35},"end":{"line":0,"character":41}},"contents":"defn helper"}}"#;
    let expected = format!(
        "Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}",
        open_response.len(),
        open_response,
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_definition_uses_latest_reopened_document() {
    ...
    let definition_body = r#"{"jsonrpc":"2.0","id":77,"method":"textDocument/definition","params":{"uri":42,"line":0,"col":38}}"#;
    let stdin = format!(
        "Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}",
        first_open_body.len(),
        first_open_body,
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_definition_uses_changed_document() {
    ...
    let definition_body = r#"{"jsonrpc":"2.0","id":78,"method":"textDocument/definition","params":{"uri":42,"line":0,"col":38}}"#;
    let stdin = format!(
        "Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}",
        open_body.len(),
        open_body,
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_references_uses_changed_document() {
    ...
    let references_body = r#"{"jsonrpc":"2.0","id":82,"method":"textDocument/references","params":{"uri":42,"line":0,"col":40}}"#;
    let stdin = format!(
        "Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}",
        open_body.len(),
        open_body,
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_rename_uses_changed_document() {
    ...
    let rename_body = r#"{"jsonrpc":"2.0","id":84,"method":"textDocument/rename","params":{"uri":42,"line":0,"col":40,"newName":"cube"}}"#;
    let stdin = format!(
        "Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}",
        open_body.len(),
        open_body,
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_hover_uses_latest_reopened_document() {
    ...
    let hover_body = r#"{"jsonrpc":"2.0","id":79,"method":"textDocument/hover","params":{"uri":42,"line":0,"col":38}}"#;
    let stdin = format!(
        "Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}",
        first_open_body.len(),
        first_open_body,
    ...
    let hover_response = r#"{"jsonrpc":"2.0","id":79,"result":{"range":{"start":{"line":0,"character":35},"end":{"line":0,"character":41}},"contents":"defn helper"}}"#;
    let expected = format!(
        "Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}",
        first_open_response.len(),
        first_open_response,
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_references_uses_latest_reopened_document() {
    ...
    let references_body = r#"{"jsonrpc":"2.0","id":83,"method":"textDocument/references","params":{"uri":42,"line":0,"col":40}}"#;
    let stdin = format!(
        "Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}",
        first_open_body.len(),
        first_open_body,
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_rename_uses_latest_reopened_document() {
    ...
    let rename_body = r#"{"jsonrpc":"2.0","id":85,"method":"textDocument/rename","params":{"uri":42,"line":0,"col":40,"newName":"cube"}}"#;
    let stdin = format!(
        "Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}Content-Length: {}\r\n\r\n{}",
        first_open_body.len(),
        first_open_body,
...
fn test_e2e_selfhost_cli_lsp_stdio_didopen_publishes_standard_type_diagnostics() {
    ...
    let if_expected = format!(
        r##""method":"textDocument/publishDiagnostics","params":{{"uri":"{if_uri}","diagnostics":[{{"range":{{"start":{{"line":0,"character":14}},"end":{{"line":0,"character":31}}}},"severity":1,"code":"LS1002","source":"lsharp","message":"if condition must be Bool"}}]}}"##
    );
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_hover_filesystem_import_schema_snapshot() {
    ...
    let hover_body = format!(
        r#"{{"jsonrpc":"2.0","id":191,"method":"textDocument/hover","params":{{"uri":200,"line":0,"col":{hover_col}}}}}"#
    );
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_definition_filesystem_import_schema_snapshot() {
    ...
    let definition_body = format!(
        r#"{{"jsonrpc":"2.0","id":193,"method":"textDocument/definition","params":{{"uri":200,"line":0,"col":{definition_col}}}}}"#
    );
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_references_filesystem_import_schema_snapshot() {
    ...
    let references_body = format!(
        r#"{{"jsonrpc":"2.0","id":194,"method":"textDocument/references","params":{{"uri":200,"line":0,"col":{references_col}}}}}"#
    );
...
fn test_e2e_selfhost_cli_main_with_lsp_stdio_rename_filesystem_import_schema_snapshot() {
    ...
    let rename_body = format!(
        r#"{{"jsonrpc":"2.0","id":195,"method":"textDocument/rename","params":{{"uri":200,"line":0,"col":{rename_col},"newName":"mid-next"}}}}"#
    );
