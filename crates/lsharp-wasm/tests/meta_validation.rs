...
//! TEST-META-02: completion marker の 3状態管理の検証
...
///
/// 移設元: repository root の `tests/meta_validation.rs`。root の `Cargo.toml` は
/// `[workspace]` のみで `[package]` を持たないため、その場所の `tests/` は
/// どの crate にも属さずコンパイルされていなかった (ISSUES.md `I-12`)。
/// TEST-META-01/03/04/05/06 と同じ本ファイルへ移し、実際に実行される状態にした。
#[test]
fn test_meta_02_completion_marker_3_states() {
    let criteria_path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../docs/development/planning/completion-criteria.md");
    let criteria_source = std::fs::read_to_string(&criteria_path)
    ...
    for i in 1..=4 {
        ...
        if criteria_source.contains(&marker) {
            ...
            assert!(has_state, "{} に個別の状態マーカーがない", marker);
        }
    }
}
