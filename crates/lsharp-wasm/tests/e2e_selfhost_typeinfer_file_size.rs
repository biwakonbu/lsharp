...
#[test]
fn e2e_selfhost_typeinfer_source_stays_within_file_size_budget() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let source = root.join("tests/e2e_selfhost_typeinfer.rs");
    let lines = line_count(&source);
    assert!(
        lines <= 800,
        "e2e_selfhost_typeinfer.rs は 500〜800 行の責務単位へ分割すること: {lines} 行"
    );
    let manifest = fs::read_to_string(&source)
        .unwrap_or_else(|error| panic!("{} 読み込み失敗: {error}", source.display()));

    let fragment_dir = root.join("tests/e2e_selfhost_typeinfer");
    ...
    let expected_includes = fragments
        ...
        .map(|path| {
            format!(
                "include!(\"e2e_selfhost_typeinfer/{}\");",
                path.file_name()
    ...
    let actual_includes = manifest
        .lines()
        .filter(|line| line.starts_with("include!(\"e2e_selfhost_typeinfer/"))
        .map(str::to_owned)
        .collect::<Vec<_>>();
    assert_eq!(
