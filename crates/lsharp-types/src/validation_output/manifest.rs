//! version 1 intent/evidence graph manifest の wire projection。
...
pub(super) fn to_manifest_json_string(graph: &IntentGraph) -> serde_json::Result<String> {
    serde_json::to_string(&ManifestWire::from_graph(graph))
}

pub(super) fn to_manifest_json_value(graph: &IntentGraph) -> serde_json::Value {
