...
#[test]
fn legacy_invariant_requires_bool_at_invariant_span() {
    const SOURCE: &str = "(defn succ [x] :invariant (+ x 1) (+ x 1))";
    let program = parse(SOURCE).expect("non-Bool legacy invariant は parse できるべき");
    ...
    assert_eq!(
        diagnostics.len(),
        1,
        "non-Bool invariant を成功扱いしてはならない"
    );
    let diagnostic = &diagnostics[0];
    assert_eq!(diagnostic.severity, Severity::Error);
    assert!(diagnostic.message.contains(":invariant は Bool 必須"));
...
#[test]
fn legacy_invariant_logic_requires_bool_operands() {
    for source in [
        "(defn succ [x] :invariant (and 1 true) (+ x 1))",
        "(defn succ [x] :invariant (or true 1) (+ x 1))",
    ] {
...
fn canonical_property_rejects_statically_true_integer_comparisons_as_vacuous() {
    for predicate in [
        ...
        let program = parse(&source)
            .expect("静的に true な property 比較は diagnostic のため parse できるべき");
...
fn canonical_property_rejects_unreachable_literal_false_precondition() {
    ...
    assert_eq!(diagnostic.severity, Severity::Error);
    assert!(diagnostic.message.contains("vacuous"), "{diagnostics:?}");
    assert_eq!(&SOURCE[diagnostic.span.start..diagnostic.span.end], "false");
    assert_eq!(diagnostic.function_name, "identity");
}
