...
impl Lower {
    ...
    fn infer_let_body_type_name(
        ...
        for (pattern, value) in bindings {
            let value_type = self.infer_expr_type_name_with_locals(&local_type_names, value);
            if let (Pattern::Var(_, name), Some(type_name)) = (pattern, value_type) {
                local_type_names.insert(name.clone(), type_name);
            }
        }
        self.infer_expr_type_name_with_locals(&local_type_names, body)
    }
        ...
        match expr {
            ...
            Expr::If(_, _, then_expr, else_expr) => self.infer_uniform_type_name(
                [
                    ...
                    self.infer_expr_type_name_with_locals(local_type_names, else_expr),
                ]
                .into_iter(),
            ),
            Expr::Match(_, _, arms) => self.infer_uniform_type_name(
                arms.iter()
                    .map(|arm| self.infer_expr_type_name_with_locals(local_type_names, &arm.body)),
            ),
            Expr::Do(_, exprs) => exprs
                .last()
                .and_then(|expr| self.infer_expr_type_name_with_locals(local_type_names, expr)),
            Expr::Let(_, bindings, body) => {
                self.infer_let_body_type_name(local_type_names, bindings, body)
            }
            // 関数呼び出しの場合、戻り値型を推定
            Expr::App(_, func, _) => {
    ...
    pub(crate) fn infer_expr_type_name_with_ctx(
        ...
        self.infer_expr_type_name_with_locals(&ctx.local_type_names, expr)
    }
