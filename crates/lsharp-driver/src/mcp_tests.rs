...
{dependency}"#,
  ...
  "modules": [
    ...
    #[cfg(unix)]
    #[test]
    fn test_package_api_tool_rejects_symlinked_src_directory() {
        use std::os::unix::fs::symlink;

        let root = std::env::temp_dir().join(format!(
            "lsharp_mcp_package_api_src_symlink_{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&root);
        let package_dir = root.join(".lsharp/packages/demo-1.0.0");
        ...
        .unwrap();
        symlink(&external_source_root, package_dir.join("src")).unwrap();
        ...
        .expect_err("package src symlink は外部 source を API に投影せず拒否するべき");
        assert!(error.contains("api.json が無く"), "{error}");
        assert!(error.contains("src/**/*.ls"), "{error}");
