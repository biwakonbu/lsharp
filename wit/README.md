...
- `wasmgc-cli-fs-streams` は `wasmgc-cli-fs` に `wasi:io/streams@0.2.3` を明示的に加えた world であり、
  ...
  `pollable.ready`（非空入力と EOF/empty input の readiness）、`wasi:io/poll.poll` の borrowed pollable list/ready index（非空/EOF input、複数 ready index、異なる input source）、空 list 入力の trap、resource-drop までを
  ...
  `input-stream.subscribe` → `pollable.block` → non-blocking `input-stream.read` の pending failure と
  `filesystem-error-code` downcast contract、
  `input-stream.subscribe` → `pollable.block` → non-blocking `input-stream.skip` の pending failure と
  `filesystem-error-code` downcast contract、
