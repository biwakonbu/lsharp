...
- `wasmgc-cli-fs-streams` は `wasmgc-cli-fs` に `wasi:io/streams@0.2.3` を明示的に加えた world であり、
  ...
  `pollable.ready`（非空入力と EOF/empty input の readiness）、`wasi:io/poll.poll` の borrowed pollable list/ready index（非空/EOF input）、resource-drop までを
  ...
  `input-stream.read` / `blocking-read` の空 list・partial read・remaining bytes・EOF/empty-source contract と、
